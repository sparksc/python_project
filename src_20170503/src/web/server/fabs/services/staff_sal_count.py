...
class staff_sal_countService():
    def add_count(self,**kwargs):
       ...
       """%(date_id)
        ...
        """
        ZhiHang_level=g.db_session.execute(ZhiHang_level).fetchone()
        ZhiHang_level=str(ZhiHang_level[0]).replace(" ",'').split(',')
        ...
        """
        Fuhang_level=g.db_session.execute(Fuhang_level).fetchone()
        Fuhang_level=str(Fuhang_level[0]).replace(" ",'').split(',')
        ...
        ZhuLi_level=g.db_session.execute(ZhuLi_level).fetchone()
        ZhuLi_level=str(ZhuLi_level[0]).replace(" ",'').split(',')
        ...
        wandian_level=g.db_session.execute(wandian_level).fetchone()
        wandian_level=str(wandian_level[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算安全防范效酬的职位' and h.HEADER_NAME='第一等级(支行)安全防范效酬的职位' ---行长(总经理),副行长(主持)
        """

        First_safe_zhihang=g.db_session.execute(First_safe_zhihang).fetchone()
        First_safe_zhihang=str(First_safe_zhihang[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算安全防范效酬的职位' and h.HEADER_NAME='副行长级别的安全防范效酬' ---副行长
        """

        First_safe_Fuhang=g.db_session.execute(First_safe_Fuhang).fetchone()
        First_safe_Fuhang=str(First_safe_Fuhang[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算安全防范效酬的职位' and h.HEADER_NAME='行长助理级别的安全防范效酬' ---行长助理
        """

        First_safe_Zhuli=g.db_session.execute(First_safe_Zhuli).fetchone()
        First_safe_Zhuli=str(First_safe_Zhuli[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算安全防范效酬的职位' and h.HEADER_NAME='第二等级安全防范的职位' ---委派会计主管(副股级),委派会计主管 ,大堂经理(享受副股级),委派风险经理,副行长(兼网点主任), 二级支行副行长(主持) ,行长助理(兼网点主任),分理处主任
        """


        Second_safe=g.db_session.execute(Second_safe).fetchone()
        Second_safe=str(Second_safe[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算安全防范效酬的职位' and h.HEADER_NAME='第三等级安全防范的职位' ---客户经理 ,大堂经理,助理会计,综合柜员 ,一般员工,见习人员,后勤人员,集中加钞员,借用人员,存款类客户经理
        """


        Third_safe=g.db_session.execute(Third_safe).fetchone()
        Third_safe=str(Third_safe[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算内后勤池付分配的职位' and h.HEADER_NAME='内后勤池付类的职位' ---助理会计,大堂经理,后勤人员,见习人员,集中加钞员,综合柜员
        """


        NeiHouqing=g.db_session.execute(NeiHouqing).fetchone()
        NeiHouqing=str(NeiHouqing[0]).replace(" ",'').split(',')
        ...
        join T_PARA_HEADER h on h.PARA_TYPE_ID=t.ID
        join T_PARA_DETAIL d on d.PARA_HEADER_ID=h.ID
        where t.TYPE_NAME='算支行二次分配的职位' and h.HEADER_NAME='支行二次分配的职位' ---派遣柜员,存款类客户经理,大堂引导员,外包人员,其他人员,试用期人员
        """

        ErciFENPEI=g.db_session.execute(ErciFENPEI).fetchone()
        ErciFENPEI=str(ErciFENPEI[0]).replace(" ",'').split(',')
        ...
        '''
        Lobby_manager=g.db_session.execute(Lobby_manager).fetchone()
        ...
        for i in his_row:
            i=list(i)
            #if i[6].split('-')[0] in ["副行长(兼网点主任)",'二级支行副行长(主持)','行长助理(兼网点主任)','分理处主任','行长(总经理)','副行长(主持)','副行长','行长助理']:
            if i[6].split('-')[0] in wandian_level+ZhiHang_level+Fuhang_level+ZhuLi_level:
                continue
            org=str(i[0])
            sale=str(i[2])
            if sale in rowlist:
        ...
        for i in result:
            ...
            if i[6].split('-')[0] in Lobby_manager_map.keys():
                ...
                if type(i[6].split('-')[0])==unicode:
                    fenlizhuren=Lobby_manager_map[i[6].split('-')[0].encode('utf-8')]+'-'+trans_level[datanfu_sql_row[0]]
                else:
                    fenlizhuren=Lobby_manager_map[i[6].split('-')[0]]+'-'+trans_level[datanfu_sql_row[0]]
                fenlichuren_sql='''
            ...
            if i[7]=='是':
                ...
                elif staff in Second_safe+Third_safe:
                    safe=sql_anquan
                else:
                    safe=0
            else:
        ...
        for i in finally_result:
            delete_hztozero='''
            delete from REPORT_MANAGER_HZSAL where org_code='%s' and sale_code='%s' and date_id=%s
            '''%(i[0],i[2],date_id)
            g.db_session.execute(delete_hztozero)
            other_sql='''
        ...
        for i in his_row_hz:
            ...
            org=str(i[0])
            sale=str(i[2])
            filter_manager_no.append(sale)#过滤员工号--历史时间存在的,v_staff_info_gdxc不允许再有
            if not i[5]:
                continue
            i[5]=i[5].split('-')[0]
            #if i[5]=='行长(总经理)' or i[5]=='副行长(主持)' or i[5]=='副行长' or i[5]=='行长助理':
            if i[5] in ZhiHang_level+Fuhang_level+ZhuLi_level:
                k=str(i[0]).split('M')
                if len(k[0])==0:
                    z='M'+k[1]
                else:
                ...
                #if i[5]=='行长(总经理)':
                if i[5] in ZhiHang_level:
                    i.append(i[5])#--11
                else:
                    i.append('行长(总经理)') #--11 i[11]用来算副行长等职位的工资(依赖行长来算的)
                if sale in rowlist_hz: #支行等级
                    continue
                else:
                    rowlist_hz[sale]=i #rowlist_hz 行长有12个长度0-11

            #elif i[5]=='副行长(兼网点主任)' or i[5]=='二级支行副行长(主持)' or i[5]=='行长助理(兼网点主任)' or i[5]=='分理处主任':
            elif i[5] in wandian_level:
                k=str(i[0]).split('M')
                if len(k[0])==0:
                    z=k[1]
                else:
                    z=k[0]
                i[0]=z #i[0]已变成算等级的机构号了
                if sale in rowlist_sec_hz: #机构网点等级排版
                    continue
                else:
                    rowlist_sec_hz[sale]=i #rowlist_sec_hz只有11个长度0-10
            else:
                pass
        ...
        for i in row_hz:
            ...
            org=str(i[0])
            sale=str(i[2])
            if sale in filter_manager_no:#说明在历史表中存在,现在的时间不允许再有了.
                continue
            if not i[5]:
                continue
            i[5]=i[5].split('-')[0]
        ...
        for i in finally_sec:
            ...
            delete from REPORT_MANAGER_HZSAL where org_code='%s' and sale_code='%s' and date_id=%s
            '''%(i[10],i[2],date_id)
            g.db_session.execute(update_hztozero)
        ...
        for i in second_HZ:
            ...
            #elif i[5]=='副行长':
            elif i[5] in Fuhang_level:
                i.append(float(dengji_sal_hz[1])*sql2_fu)#--职务工资12
                i.append(float(dengji_sal_hz[2])*sql2_fu)#职级(位)13
            elif i[5] in ZhuLi_level:
                i.append(float(dengji_sal_hz[1])*sql_zhu)#--职务工资12(记得加行长助理)
                i.append(float(dengji_sal_hz[2])*sql_zhu)#职级(位)13
            else :
                i.append(float(0)) #--职务工资12(记得加行长助理)
                i.append(float(0))#职级(位)13
            if i[9]=='是':
                i[13]=float(i[13])*sql_shiping
            if i[6]=='是':
                staff=i[5].split('-')[0]
                #if staff=='行长(总经理)'or staff=='副行长(主持)':
                if staff in First_safe_zhihang:
                    safe=sql_yiji+sql_anquan
                #elif staff=='行长助理':
                elif staff in First_safe_Zhuli:
                    safe=sql_yiji*sql_zhu+sql_anquan
                #elif staff=='副行长':
                elif staff in First_safe_Fuhang:
                    safe=sql_yiji*sql2_fu+sql_anquan
                else:
                    safe=sql_anquan
            else:
        ...
        for i in finally_hz:
            other_sqltozero='''
            update REPORT_MANAGER_OTHER set BASE_PAY=0 , POSITION_PAY=0 , SAFE_FAN_SAL=0 , OTHER_SAL1_DUAN =0 where ORG_CODE='%s' and  SALE_CODE ='%s' and date_id=%s 
            '''%(i[10],i[2],date_id)
            g.db_session.execute(other_sqltozero)
            sql_is_value_hz="""
            ...
            if is_value:
                ...
                sql_insert_hz_max_id='''
                select nvl(max(id),0)+1 from REPORT_MANAGER_HZSAL
                '''
                sql_insert_hz_max_id=g.db_session.execute(sql_insert_hz_max_id).fetchone()
                sql_insert_hz='''
                insert into REPORT_MANAGER_HZSAL(id,date_id,org_code,org_name,sale_code,SALE_NAME,HZ_BASE_PAY,HZ_POSITION_PAY,HZ_SAFE_FAN_SAL) 
                values(%s,%s,'%s','%s','%s','%s',%s,%s,%s)
                '''%(sql_insert_hz_max_id[0],date_id,i[10],i[1],i[2],i[3],int(Decimal(i[12])*100),int(Decimal(i[13])*100),int(Decimal(i[14])*100))
        ...
        day_row_list={}
        branch_sec_rowList={}
        com_branch_manager_no=[]
        for i in his_com_branch:
            i=list(i)
            sale=str(i[2])
            com_branch_manager_no.append(sale)#通过历史记录确定员工号,筛选
            if not i[5]:
                continue
            his_pos=i[5].split('-')[0]
            #if his_pos in ['助理会计','大堂经理','后勤人员','见习人员','集中加钞员','综合柜员']:
            if his_pos in NeiHouqing:
                org=str(i[0])
                sale=str(i[2])
                if sale in day_row_list:
                    continue;
                else:
                    day_row_list[sale]=i
        ...
        for i in now_com_branch:
            i=list(i)
            sale=str(i[2])
            if sale in com_branch_manager_no:
                continue
            if not i[5]:
                continue
            his_pos=i[5].split('-')[0]
        ...
        for i in his_com_branch:#二次分配
            ...
            if his_pos in ErciFENPEI:
                org=str(i[0])
                sale=str(i[2])
                if sale in branch_sec_rowList:
                    continue;
                else:
                    branch_sec_rowList[sale]=i
        ...
        for i in now_com_branch:
            ...
            if his_pos in ErciFENPEI:
                ...
                if sale in branch_sec_rowList:
                    ...
                    branch_sec_rowList[sale]=i#二次分配
    ...
    def edit_save(self,**kwargs):
        ...
        if zw in ZhiHang_level+Fuhang_level+ZhuLi_level:
            edit_update_sql='''
            update REPORT_MANAGER_HZSAL set HZ_BASE_PAY=%s,HZ_POSITION_PAY=%s,HZ_SAFE_FAN_SAL=%s where org_code='%s' and sale_code='%s' and date_id=%s    
