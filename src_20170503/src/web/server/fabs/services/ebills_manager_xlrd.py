...
class Ebills_ManagerService():
    ...
    def ebills_hook_upload(self, filepath,filename):
        ...
        for r in range(4,nrows):
            try:
                MONTH= str(sheet.cell(r,2).value).replace('.0',"").replace('.00',"").strip()
                ORG_CODE= str(sheet.cell(r,0).value).replace('.0',"").replace('.00',"").strip()
                ORG_NAME= str(sheet.cell(r,1).value)
