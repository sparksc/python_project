...
class Ebills_ManagerService():
    ...
    def internation_count(self,**kwargs):
        ...
        if len(cust_in_noinfo)>1:
            cust_in_noinfo=repr(tuple(cust_in_noinfo))
        elif len(cust_in_noinfo)==1:
            cust_in_noinfo=repr((cust_in_noinfo[0],'0'))
        else:
            cust_in_noinfo=repr(('0','0'))
        ...
        and IN_CST_NO not in %s
        group by LAUNCHDATE,f.IN_CST_NO,CORP_NAME,IN_CST_NO,CORPNO,BELONGORGNO,BRANCH_NAME
        """%(str(month),date_id,cust_in_noinfo)
        ...
        and IN_CST_NO is not null
        and IN_CST_NO not in %s
        group by finisheddate,f.IN_CST_NO,CORP_NAME,IN_CST_NO,CORPNO,BELONGORGNO,BRANCH_NAME
        """%(str(month),date_id,cust_in_noinfo)
        ...
        and IN_CST_NO not in %s
        group by LAUNCHDATE,f.IN_CST_NO,CORP_NAME,IN_CST_NO,CORPNO,BELONGORGNO,BRANCH_NAME
        """%(devlop_base,str(month),date_id,cust_in_noinfo)
