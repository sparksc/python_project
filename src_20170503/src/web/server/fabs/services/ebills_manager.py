...
class Ebills_ManagerService():
    ...
    def ebills_hook_upload(self, filepath,filename):
        ...
        try:
            ...
            #行
            nrows = sheet.max_row
            current_app.logger.debug(sheet.max_column)
            if nrows <5:
                raise Exception(u"警告:文件为空")
            if sheet.max_column!=10:
                raise Exception(u"警告:文件列数不对")
        except Exception,e:
            return str(e)
        f_user_dict={}
        ...
        for r in range(5,nrows+1):
            try:
                ORG_CODE= str(sheet.cell('A'+str(r)).value).replace('.0',"").replace('.00',"").strip()
                if ORG_CODE in ['小计','总计']:
                    continue
                ORG_NAME= str(sheet.cell('B'+str(r)).value)
                MONTH= str(sheet.cell('C'+str(r)).value).replace('.0',"").replace('.00',"").strip()
                CUST_IN_NO= str(sheet.cell('D'+str(r)).value).replace('.0',"").replace('.00',"").strip()
                CORPNAME= str(sheet.cell('E'+str(r)).value)
                SALE_CODE= str(sheet.cell('F'+str(r)).value).replace('.0',"").replace('.00',"").strip()
                SALE_NAME= str(sheet.cell('G'+str(r)).value)
                TOTAL_SET= str(sheet.cell('H'+str(r)).value).replace(',','')
                TOTAL_CROSS= str(sheet.cell('I'+str(r)).value).replace(',','')
                IS_OPEN_NEW_WH= str(sheet.cell('J'+str(r)).value)

                if MONTH.strip() in ['','None']:
                    raise Exception(u'请填写月份')
                if ORG_CODE.strip()  in ['','None']:
                    raise Exception(u'请填写机构号')
                if CUST_IN_NO.strip() in ['','None']:
                    raise Exception(u'请填写客户内码')
                if CORPNAME.strip() in ['','None']:
                    raise Exception(u'请填写客户名')
                if IS_OPEN_NEW_WH.strip()=="" or IS_OPEN_NEW_WH==None or IS_OPEN_NEW_WH=='None':
                    raise Exception(u'请填写是否新增有效户')
                if ORG_CODE in org_dict:
                    ORG_NAME=org_dict.get(ORG_CODE)
                else:
                    raise Exception(u'第%s行此机构号"%s"不存在'%(r,ORG_CODE))
    ...
    def ebills_hook_cunkuan_upload(self, filepath,filename):
        ...
        for r in range(5,nrows+1):
            try:
                ...
                CUST_IN_NO= str(sheet.cell('D'+str(r)).value).replace('.0',"").replace('.00',"").strip()
                CORPNAME= str(sheet.cell('E'+str(r)).value)
                BALANCE= str(sheet.cell('F'+str(r)).value).replace(',','')
                MONTHAMT= str(sheet.cell('G'+str(r)).value).replace(',','')
                DAYAMT= str(sheet.cell('H'+str(r)).value).replace(',','')
    ...
    def internation_count(self,**kwargs):
        ...
        using
        (
        select d.ym,max(month_days),org_no,org_name,e.cust_in_no,cust_name,((sum(nvl(balance,0))*1.0))/ max(d.month_days) as avn_month,sale_code,sale_name   from report_sale_rmb_exg e 
        join d_date d on d.id=e.date_id
        where d.id>=%s and d.id<=%s
        group by d.ym,org_no,org_name,e.cust_in_no,cust_name,sale_code,sale_name) a 
        on   ---月日均
        (eh.month=%s and eh.org_code=a.org_no  and eh.cust_in_no=a.cust_in_no and eh.sale_code=a.sale_code and eh.sale_name=a.sale_name)
        ...
        '''%(int(str(date_id)[:6]+'01'),date_id,month,month)
        ...
        using
        (
        select d.year,(select beg_year_days from d_date where id=%s),org_no,org_name,e.cust_in_no,cust_name,(sum(nvl(balance,0))*1.0)/ (select beg_year_days from d_date where id=%s)  as avg_year  ,sale_code,sale_name from report_sale_rmb_exg e 
        join d_date d on d.id=e.date_id
        where d.id>=%s and d.id<=%s
        group by d.year,org_no,org_name,e.cust_in_no,cust_name ,sale_code,sale_name
        ) a on   ---年日均
        (eh.month=%s and eh.org_code=a.org_no  and eh.cust_in_no=a.cust_in_no and eh.sale_code=a.sale_code and eh.sale_name=a.sale_name)
        ...
        '''%(date_id,date_id,int(str(date_id)[:4]+'0101'),date_id,month,month)
