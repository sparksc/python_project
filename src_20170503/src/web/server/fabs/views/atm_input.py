...
from ..services import Atm_inputService
from . import route


bp = Blueprint('atm_input', __name__, url_prefix='/atm_input')

atm_inputService = Atm_inputService()


@route(bp, '/add_save',methods=['POST'])
def add_save():
    return atm_inputService.add_save(**request.json)
@route(bp, '/edit_save',methods=['POST'])
def edit_save():
    return atm_inputService.edit_save(**request.json)
...
def upload():
    ...
    return atm_inputService.upload(local_path)
