...
bp = Blueprint('counter_fine_amount', __name__, url_prefix='/counter_fine_amount')

counterFineAmountService = CounterFineAmountService()
'''
柜员扣罚金额手工维护
'''
@route(bp, '/fine_save',methods=['POST'])
def fine_save():
    return counterFineAmountService.fine_save(**request.json)

@route(bp, '/fine_delete',methods=['POST'])
def fine_delete():
    return counterFineAmountService.fine_delete(**request.json)

@route(bp, '/fine_update',methods=['POST'])
def fine_update():
    return counterFineAmountService.fine_update(**request.json)

@route(bp, '/fine_upload',methods=['POST'])
def fine_upload():
    ...
    return counterFineAmountService.fine_upload(local_path,file1.filename)
