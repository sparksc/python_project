ysp.controller('contractRegistrationController', function($scope, $rootScope, LOCAService, CustomerSearchService ){
    ...
    /* Query Application */
    $scope.tableHead=['客户名称' , '业务品种' , '币种'  ,'批复金额' , '主要担保方式'  , '申请人', '申请机构', '操作'];
    $scope.queryCond = {};
    $scope.tableData = [];
    ...
    $scope.submitApplication = function(applicationId){
        ...
        LOCAService.detail(applicationId).success(function(resp){
            ...
            for(var i in resp.data){
                var name = '';
                var completionStatus = '未知';
                var showClass = '';

                switch(i){
                    case 'sign_comment_flag':
                        name = '是否签署意见'; break;
                    case 'write_survey_report_flag':
                        name = '是否签署意见'; break;
                    default:
                        name = '未知项';
                }
                ...
                switch(resp.data[i]){
                    case true:
                       completionStatus = '通过'; break 
                    case false:
                       completionStatus = '不通过'; break 
                }

                switch(completionStatus){
                    case '通过':
                        showClass = 'success'; break;
                    case '注意':
                        showClass = 'info'; break;
                    case '警告':
                        showClass = 'warning'; break;
                    case '不通过':
                        showClass = 'danger'; break;


                }
                $scope.submitTableData.push({'name':name,'completionStatus':completionStatus, 'showClass':showClass});
            }
        });
    };
