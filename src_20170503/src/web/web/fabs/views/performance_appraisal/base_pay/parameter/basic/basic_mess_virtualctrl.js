 ...
 */
function basic_messvirtualController($scope, $rootScope, $filter, SqsReportService, staffrelationService, staff_statusService,branchmanageService,permissionService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search.IS_VIRTUAL = '是'
    ...
    find_branchs();   
    var element_add = angular.element('#myModaladd_virtual');
    $scope.add = function(){
        $scope.add_date={};
        ...
        $scope.add_date.x6 = "正式工";
        $scope.add_date.x7 = null;
        $scope.add_date.x8 = '客户经理';
        $scope.add_date.x9 = null;
        $scope.add_date.x10 = '客户经理';
        $scope.add_date.x11 = '否';
        $scope.add_date.x12 = '在职';
        $scope.add_date.x13 = '否';
        $scope.add_date.x14 = '是';
    ...
    $scope.add_save = function (valid){
        ...
        staffrelationService.tsave({'add_date':$scope.add_date}).success(function (resp){
            var params_username={};
            params_username['USER_NAME']=$scope.add_date.x4
            SqsReportService.info('userpermission',params_username).success(function(resp_id)           {
                permissionService.user_permission_group_save({ //增加权限管理为支行客户经理组
                'old_groups':[],
                'user_id':resp_id.rows[0][7],
                'groups':[1015]
    ...
    $scope.detail = function(row){
        ...
        $('#virtual_history_modal').modal('show');
    };
    ...
    $scope.show_lt_modal = function(trigger_elem){
        $("#tab_"+ $scope.tabId + "_content").find("div[name='virtual_loan_type_modal']").modal("show");
    }
...
basic_messvirtualController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService' ,'staffrelationService','staff_statusService','branchmanageService','permissionService'];

angular.module('YSP').service('basic_messvirtualController', basic_messvirtualController);
