 ...
 */
function manscoController($scope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_sco?export=1";
        ...
        SqsReportService.info('man_sco',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_sco?export=1"+"&S_DATE="+moment().format('YYYYMMDD');
};

manscoController.$inject = ['$scope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];
