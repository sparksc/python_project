 ...
 */
function counterExamNumController($scope,store, $filter, $rootScope, SqsReportService,branchmanageService,counterFineAmountService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/counter_exam_num?export=1";
        ...
        console.log(params)
        SqsReportService.info('counter_exam_num',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
        if(resp.rows[0]=='未'){
            alert("未到月末,不能查看")
    ...
    var add_element=angular.element('#counter_exam_num__modal');
    var add_button_element=angular.element('#counter_exam_num_new_add_button')
    var edit_button_element=angular.element('#counter_exam_num_save_edit_button')
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/counter_exam_num?export=1"+"&DATE_ID="+moment().format('YYYYMMDD');
    ...
    $scope.save = function(){
        ...
        nsd["assess_man_num"]=$scope.newstaffdata[3];
        console.log(nsd)
        var nd = {"newdata":nsd};
        counterFineAmountService.exam_save(nd).success(function (reps){
    ...
    $scope.edit_save = function(){
        ...
        var nd = {"newdata":nsd};
        counterFineAmountService.exam_update(nd).success(function (reps){
    ...
    if(files.length==0)
        ...
        $.ajax({
        type: "POST",
        url : base_url+"/counter_fine_amount/exam_upload/",
        data: form,
        processData: false,
        contentType: false,
...
counterExamNumController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branchmanageService','counterFineAmountService'];

angular.module('YSP').service('counterExamNumController', counterExamNumController);
