 ...
 * Khjlanzb Controller
 */
function orgcreditController($scope, $rootScope, $filter, SqsReportService,store,branchmanageService,depappointService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    function do_search(){
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_credit_card?export=1a";
        ...
        SqsReportService.info('org_credit_card', params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_credit_card?export=1a"+"&tdate="+moment().format('YYYYMMDD');
    ...
    $scope.ztreeBtmConfirm = function(){
        ...
        $scope.cust_search.jgbhh = res_msg;
...
orgcreditController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','store','branchmanageService','depappointService'];
angular.module('YSP').service('orgcreditController', orgcreditController);
