/**
 * account_bank Controller
 */
ysp.controller("ebank_replaceController",function ($scope,store, $filter, $rootScope, SqsReportService, ebank_replaceService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebank_replace?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    $scope.newstaffdata= {};    
    ...
    */    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/ebank_replace/upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        if(params["DATE"]){
            if(!$scope.filterInt($scope.cust_search.DATE)){
                alert("请输入合法日期，如20170310");
                $("div[name='loading']").modal("hide");
                $scope.cust_search.DATE='';
                return;
            }
        }
        //console.log(params);
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('ebank_replace', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
    ...
    $scope.add = function(){
        ...
        $scope.newstaffdata[8]='';
        $scope.newstaffdata[9]='';
        $scope.newstaffdata[10]='';
        $scope.newstaffdata[11]='';
        $scope.newstaffdata[12]='';
        $scope.newstaffdata[13]='';
        $('#ebank_replace_modal').modal('show');
        $('#ebank_replace_new_add_button').show();
        $('#ebank_replace_save_edit_button').hide();
    };
    ...
    $scope.add_save = function(){
       // console.log($scope.newstaffdata)
        var nsd ={};
        if($scope.newstaffdata[0] == ''||$scope.newstaffdata[1] == ''  || $scope.newstaffdata[2] == '' || $scope.newstaffdata[3] == '' || $scope.newstaffdata[4] == '' || $scope.newstaffdata[5] == '' || $scope.newstaffdata[6]=='' || $scope.newstaffdata[7]==''|| $scope.newstaffdata[8]==''|| $scope.newstaffdata[9]==''|| $scope.newstaffdata[10]==''|| $scope.newstaffdata[11]==''|| $scope.newstaffdata[12]==''){
            alert("输入不能为空");
            return;
        }
        ...
        nsd["mis_rank"]=$scope.newstaffdata[5];
        nsd["basic_rank"]=$scope.newstaffdata[6];
        nsd["ryear"]=$scope.newstaffdata[7];
        nsd["score_rank"]=$scope.newstaffdata[8];
        nsd["work_rank"]=$scope.newstaffdata[9];
        nsd["repleace_rank"]=$scope.newstaffdata[10];
        nsd["skill"]=$scope.newstaffdata[11];
        nsd["experience"]=$scope.newstaffdata[12];
        nsd["remarks"]=$scope.newstaffdata[13];
        var org_code = $scope.newstaffdata[1] ,
            user_code = $scope.newstaffdata[3]
        params = {'BRANCH_CODE':org_code,'USER_CODE':user_code};
        //console.log(params)
        SqsReportService.info('ebank_replace', params).success(function(resp) {
            $scope.data = resp;
            ...
            if (($scope.data.rows || []).length > 0) {
                $scope.tableMessage = "";
                $scope.search()
                $('#ebank_replace_modal').modal('hide');
                alert("该员工已存在");
            }else{
                $scope.tableMessage = ""
                console.log(nsd)
                var nd = {"newdata":nsd};
                ebank_replaceService.add_save(nsd).success(function (reps){
                    $scope.search(); 
                    $('#ebank_replace_modal').modal('hide');
                    alert(reps.data)
                });
    ...
    $scope.edit = function(row){
        $('#ebank_replace_edit_modal').modal('show');
        $('#ebank_replace_new_add_button').hide();
        $('#ebank_replace_save_edit_button').show();
    ...
    $scope.edit_save = function(){
        ...
        nsd["experience"]=$scope.newstaffdata[12];
        nsd["remarks"]=$scope.newstaffdata[13];
        nsd["item_id"]=$scope.newstaffdata[14];
        /*
        var nd = {"newdata":nsd};*/
        ebank_replaceService.edit_save(nsd).success(function (reps){
             $scope.search(); 
             $('#ebank_replace_edit_modal').modal('hide');
             alert(reps.data)
        });
    ...
    $scope.del = function(item){
         ...
         if(confirm("确认删除？")){
             console.log(item[14]);     
             ebank_replaceService.del({"item_id":item[14]}).success(function (reps){
             //console.log(reps) 
                 $scope.search(); 
                 $('#ebank_replace_modal').modal('hide');
                 alert(reps.data)
     ...
     $scope.find_users_by_branches = function(){
        branchmanageService.find_users_by_branches({'branch_id':$scope.cust_search.BRANCH_CODE}).success(function(reps){
            $scope.model2 =reps.data;
       });
    };
    ...
    $scope.choose_branch_type = function(branch_code, branch_name, branch_level){
        $scope.cust_search.BRANCH_CODE= branch_code.replace(/(^\s*)|(\s*$)/g, "");
        ...
        console.log($scope.cust_search.BRANCH_CODE); 
    ...
    $scope.ztreeBtmConfirm = function(){
        ...
        $scope.cust_search.BRANCH_CODE= res_msg;
