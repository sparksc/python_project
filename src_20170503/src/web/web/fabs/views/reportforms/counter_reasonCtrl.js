 ...
 */
function counterReasonCtrlController($scope,store, $filter, $rootScope, SqsReportService,branchmanageService,countexam_basevolservice) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/counter_reason?export=1";
        ...
        console.log(params)
        SqsReportService.info('counter_reason',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
        if(resp.rows[0]=='未'){
            alert("未到月末,不能查看")
    ...
    var add_element=angular.element('#counter_reason__modal');
    var add_button_element=angular.element('#counter_reason_new_add_button')
    var edit_button_element=angular.element('#counter_reason_save_edit_button')
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/counter_reason?export=1"+"&DATE_ID="+moment().format('YYYYMMDD');
    ...
    $scope.save = function(){
        ...
        console.log(nsd)
        var nd = {"newdata":nsd};
        countexam_basevolservice.counter_reason_save(nd).success(function (reps){
    ...
    $scope.edit_save = function(){
        ...
        var nd = {"newdata":nsd};
        countexam_basevolservice.counter_reason_update(nd).success(function (reps){
    ...
    nsd["id"]=row[8];
    var nd = {"newdata":nsd};
        countexam_basevolservice.counter_reason_delete(nd).success(function (reps){
        //console.log(reps) 
        $scope.search(); 
        add_element.modal('hide');
        });
    ...
    if(files.length==0)
        ...
        $.ajax({
        type: "POST",
        url : base_url+"/countexam_basevol_hander/counter_reason_upload",
        data: form,
        processData: false,
        contentType: false,
...
counterReasonCtrlController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branchmanageService','countexam_basevolservice'];

angular.module('YSP').service('counterReasonCtrlController', counterReasonCtrlController);
