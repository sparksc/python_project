 ...
 * Mandep Controller
 */
function quarter_term_sale_mbank_result_sumController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search.CHECK_DATE_ID = moment();
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank_result_sum?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") +"&CHECK_DATE_ID=" + $scope.cust_search.CHECK_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code
        for(var key in $scope.cust_search){
            ...
            if(key=='CHECK_DATE_ID')params['CHECK_DATE_ID']=params['CHECK_DATE_ID'].format("YYYYMMDD");
            $scope.excelurl = $scope.excelurl + "&"+key+"="+params[key]
            }
        }

        SqsReportService.info('quarter_term_sale_mbank_result_sum',params).success(function(resp) {
...
quarter_term_sale_mbank_result_sumController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('quarter_term_sale_mbank_result_sumController', quarter_term_sale_mbank_result_sumController); 
