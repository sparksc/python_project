 ...
 */
function managerNumController($scope,store, $filter, $rootScope, SqsReportService,branchmanageService,hallManagerHanderServer) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/manager_num?export=1";
        ...
        console.log(params)
        SqsReportService.info('manager_num',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
        if(resp.rows[0]=='未'){
            alert("未到月末,不能查看")
    ...
    var add_element=angular.element('#manager_num__modal');
    var add_button_element=angular.element('#manager_num_new_add_button')
    var edit_button_element=angular.element('#manager_num_save_edit_button')
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/manager_num?export=1"+"&DATE_ID="+moment().format('YYYYMMDD');
    ...
    $scope.save = function(){
        ...
        if(Number($scope.newstaffdata[3])<0)
        {   
            $scope.newstaffdata[3]=null
            alert("大堂经理人数不能为负数")
            return
        }
        if(Number($scope.newstaffdata[3]).toString()=='NaN')
        {   
            $scope.newstaffdata[3]=null
            alert("大堂经理人数存在非法字符")
            return
        ...
        nsd["manager_num"]=$scope.newstaffdata[3];
        if ($scope.newstaffdata[4]==null || $scope.newstaffdata[4]==""){
        $scope.newstaffdata[4]=""
        }
        nsd["manager_bak"]=$scope.newstaffdata[4];
        console.log(nsd)
        var nd = {"newdata":nsd};
        hallManagerHanderServer.manager_num_save(nd).success(function (reps){
    ...
    $scope.edit_save = function(){
        ...
        var nd = {"newdata":nsd};
        hallManagerHanderServer.manager_num_update(nd).success(function (reps){
    ...
    nsd["id"]=row[6];
    var nd = {"newdata":nsd};
        hallManagerHanderServer.manager_num_delete(nd).success(function (reps){
        //console.log(reps) 
        $scope.search(); 
        add_element.modal('hide');
        });
    ...
    if(files.length==0)
        ...
        $.ajax({
        type: "POST",
        url : base_url+"/hall_manager_hander/manager_num_upload",
        data: form,
        processData: false,
        contentType: false,
...
managerNumController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branchmanageService','hallManagerHanderServer'];

angular.module('YSP').service('managerNumController', managerNumController);
