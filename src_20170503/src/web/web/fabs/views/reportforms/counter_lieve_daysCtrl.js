 ...
 */
function counterLieveDaysController($scope,store, $filter, $rootScope, SqsReportService,branchmanageService,counterFineAmountService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/counter_lieve_days?export=1";
        ...
        console.log(params)
        SqsReportService.info('counter_lieve_days',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
        if(resp.rows[0]=='未'){
            alert("未到月末,不能查看")
    ...
    var add_element=angular.element('#counter_lieve_days__modal');
    var add_button_element=angular.element('#counter_lieve_days_new_add_button')
    var edit_button_element=angular.element('#counter_lieve_days_save_edit_button')
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/counter_lieve_days?export=1"+"&DATE_ID="+moment().format('YYYYMMDD');
    ...
    $scope.save = function(){
        ...
        nsd["counter_lieve_days"]=$scope.newstaffdata[5];
        var nd = {"newdata":nsd};
        counterFineAmountService.lieve_save(nd).success(function (reps){
    ...
    $scope.edit_save = function(){
       ...
       console.log($scope.newstaffdata)
        ...
        var nd = {"newdata":nsd};
        counterFineAmountService.lieve_update(nd).success(function (reps){
    ...
    if(files.length==0)
        ...
        $.ajax({
        type: "POST",
        url : base_url+"/counter_fine_amount/lieve_upload/",
        data: form,
        processData: false,
        contentType: false,
...
counterLieveDaysController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branchmanageService','counterFineAmountService'];

angular.module('YSP').service('counterLieveDaysController', counterLieveDaysController);
