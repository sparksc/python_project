 ...
 * Finsum Controller
 */
function FinsumController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        if($scope.cust_search.remark==null||$scope.cust_search.remark=='') 
        {
            alert('请先输入要查询的理财产品');
            return; 
        } 
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/fin_sum?export=1" + "&login_branch_no=" + $rootScope.user_session.branch_code;
        ...
        SqsReportService.info('fin_sum',params).success(function(resp) {
...
FinsumController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('FinsumController', FinsumController); 
