 ...
 * Orgloan Controller
 */
function OrgloanController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_loan?export=1a";
        ...
        SqsReportService.info('org_loan',params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_loan?export=1a"+"&S_DATE="+moment().format('YYYYMMDD');
...
OrgloanController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('OrgloanController', OrgloanController); 
