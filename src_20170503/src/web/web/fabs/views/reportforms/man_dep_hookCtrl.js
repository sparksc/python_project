 ...
 * Mandep Controller
 */
function ManDepHookController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_dep_hook?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") +"&END_DATE_ID="+ $scope.cust_search.END_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code; 
    ...
    $scope.search = function() {
        ...
        if($scope.cust_search.END_DATE_ID<$scope.cust_search.FROM_DATE_ID){
            alert("起始日期不能小于终止日期")
            return;
        }
        if(moment($scope.cust_search.FROM_DATE_ID).format('YYYY') !=moment($scope.cust_search.END_DATE_ID).format('YYYY')){
        alert("查询时间不能跨年")
        return
        } 
        var a=moment($scope.cust_search.END_DATE_ID).subtract(30,'days').format('YYYYMMDD')
        var b=moment($scope.cust_search.FROM_DATE_ID).format('YYYYMMDD')
        ...
        SqsReportService.info('man_dep_hook',params).success(function(resp) {
...
ManDepHookController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('ManDepHookController', ManDepHookController); 
