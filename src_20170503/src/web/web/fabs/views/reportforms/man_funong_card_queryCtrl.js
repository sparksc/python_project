 ...
 *  Controller
 */
function ManFunongCardQueryController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search.FROM_DATE_ID = moment();
    $scope.cust_search.END_DATE_ID = moment();
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_funong_card?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") +"&END_DATE_ID="+ $scope.cust_search.END_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code; 
    $scope.excelurl1 = rpt_base_url + "/report_proxy/sqs/man_funong_card_query?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") +"&END_DATE_ID="+ $scope.cust_search.END_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code; 
    ...
    $scope.onAction = function(conversation_id, action) {
        SqsReportService.action(conversation_id, action).success(function(resp) {
            ...
            merge_table_show( "man_funong_card_query", $scope.data.merge_cols)
        });
    };   
    ...
    $scope.search = function() {
        if(!$scope.cust_search.FROM_DATE_ID)
        {
            alert('请选择开始日期!');
            return;
        }

        if(!$scope.cust_search.END_DATE_ID)
        {
            alert('请选择终止日期!');
            return;
        }
        ...
        for(var key in $scope.cust_search){
            ...
            if(key=='FROM_DATE_ID')
                params['FROM_DATE_ID']=params['FROM_DATE_ID'].format("YYYYMMDD");
            if(key=='END_DATE_ID')
                params['END_DATE_ID']=params['END_DATE_ID'].format("YYYYMMDD");
            $scope.excelurl = $scope.excelurl + "&"+key+"="+params[key]
            $scope.excelurl1 = $scope.excelurl1 + "&"+key+"="+params[key]
            }
        }

        SqsReportService.info('man_funong_card',params).success(function(resp) {
...
ManFunongCardQueryController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('ManFunongCardQueryController', ManFunongCardQueryController); 
