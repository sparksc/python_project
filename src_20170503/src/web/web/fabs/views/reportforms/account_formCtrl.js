/**
 * account_formController
 */
function account_formController($scope,$rootScope, $filter, SqsReportService,store,branchmanageService,depappointService,account_formService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    function do_search(){
        ...
        SqsReportService.info('account_form', params).success(function(resp) {
    ...
    $scope.calculate = function(){
        $("div[name='loading']").modal("show");
        account_formService.calculate({}).success(function(resp){
            $scope.search();
            alert(resp.data);
            $("div[name='loading']").modal("hide");
        });                                                        
    ...
    $scope.affirm = function(){
        $("div[name='loading']").modal("show");
        account_formService.affirm({}).success(function(resp){
            $scope.search();
            alert(resp.data);
            $("div[name='loading']").modal("hide");
       }).error(function(resp){
    ...
    $scope.change_level = function(row){
         $('#account_form_modal').modal('show');
         $('#account_form_new_add_button').hide();
         $('#account_form_save_edit_button').show();
         console.log(row);
         $scope.newstaffdata[0] = row[47];
         $scope.newstaffdata[1] = row[50];
    ...
    $scope.change_save = function(){
        ...
        console.log(nsd);
        account_formService.change_save(nsd).success(function (reps){
            $scope.search();
            $('#account_form_modal').modal('hide');
            alert(reps.data)
        });
    };
...
account_formController.$inject = ['$scope', '$rootScope','$filter', 'SqsReportService','store','branchmanageService','depappointService','account_formService'];
angular.module('YSP').service('account_formController', account_formController);
