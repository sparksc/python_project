 ...
 */
function ebankplanparameterController($scope,store, $filter, $rootScope, SqsReportService,villageinputService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebank_plan_parameter?export=1"
    $scope.newstaffdata= {};    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/villageinput/ebank_upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('ebank_plan_parameter', params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.save = function(){
        ...
        var nsd ={};
        nsd["YEAR"]=$scope.newstaffdata[0].format('YYYY');
        nsd["third_org_code"]=$scope.newstaffdata[1];
        nsd["third_org_name"]=$scope.newstaffdata[2];
        nsd["org_code"]=$scope.newstaffdata[3];
        nsd["org_name"]=$scope.newstaffdata[4];
        nsd["manager_code"]=$scope.newstaffdata[5];
        nsd["manager_name"]=$scope.newstaffdata[6];
        nsd["tar_sj"]=$scope.newstaffdata[7];
        nsd["tar_wy"]=$scope.newstaffdata[8];
        nsd["tar_pos"]=$scope.newstaffdata[9];
        nsd["tar_etc"]=$scope.newstaffdata[10];
        nsd["tar_epay"]=$scope.newstaffdata[11];
        nsd["tar_zn"]=$scope.newstaffdata[12];
        console.log(nsd)
        var nd = {"newdata":nsd};
        villageinputService.ebank_save(nd).success(function (reps){
        $scope.search(); 
    ...
    $scope.edit_save = function(){
        ...
        nsd["tar_epay"]=$scope.newstaffdata[11];
        nsd["tar_zn"]=$scope.newstaffdata[12];
        nsd["id"]=$scope.newstaffdata[13];
        var nd = {"newdata":nsd};
        villageinputService.ebank_update(nd).success(function (reps){
        $scope.search(); 
        $('#villageinput_modal').modal('hide');
        });
    ...
    nsd["id"]=row[13];
    var nd = {"newdata":nsd};
        villageinputService.ebank_delete(nd).success(function (reps){
        //console.log(reps) 
        $scope.search(); 
        $('#villageinput_modal').modal('hide');
        });
...
ebankplanparameterController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','villageinputService','branchmanageService'];

angular.module('YSP').service('ebankplanparameterController', ebankplanparameterController);
