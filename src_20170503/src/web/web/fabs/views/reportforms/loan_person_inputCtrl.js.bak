 ...
 */
ysp.controller('loanpersoninputController', function($scope, $rootScope,  handInputService,SqsReportService,loaninputService,branchmanageService){
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('loan_person', params).success(function(resp) {
            $scope.data = resp;
    ...
    var element_edit = angular.element('#loan_person_edit');
    var element_add = angular.element('#loan_person_add');
    ...
    $scope.add_save = function (){
        ...
        loaninputService.save({'add_date':$scope.add_date}).success(function (resp){
            alert(resp.data);
            $scope.search();
    ...
    $scope.del = function(item){
		var r=confirm("确定删除？");
		if(r==true){
           loaninputService.delt({'item_id':item[14]}).success(function(resp){
              alert(resp.data);
              $scope.search();
           });
        }
    ...
    $scope.edit = function(item){
        ...
        $scope.up_date.person_loan_name=item[7]
        $scope.up_date.person_assuer_name=item[8]
        $scope.up_date.cop_present_name=item[9]
        $scope.up_date.head_photo=item[10]
        $scope.up_date.finger_print=item[11]
        $scope.up_date.id_card=item[12]
        $scope.up_date.marriage=item[13]
    ...
    $scope.edit_save = function (){
            ...
            loaninputService.edit({'up_date':$scope.up_date}).success(function(resp){
