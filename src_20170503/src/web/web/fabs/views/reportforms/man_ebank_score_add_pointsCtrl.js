/**
 * man_ebank_score_add_points Controller
 */
function man_ebank_score_add_pointsController($scope,store, $filter, $rootScope, SqsReportService,villageinputService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_ebank_score_add_points?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('man_ebank_score_add_points', params).success(function(resp) {
    ...
    $scope.add = function(){
        ...
        $('#man_ebank_score_add_points').modal('show');
    };
    $scope.save = function(){
        if($scope.kyear == ''||$scope.org_no==''||$scope.org_name==''||$scope.org_name==''||$scope.user_name==''||$scope.name==''||$scope.add_points==''){
            alert("信息未填完全,请检查!");
            return;
        }
        ...
        if(!($scope.filterNum($scope.add_points))){
            alert("附加分请输入合法数值");
            return;
        }

       
        villageinputService.ebank_add_save({'kyear':$scope.kyear,'org_no':$scope.org_no,'org_name':$scope.org_name,'user_name':$scope.user_name,'name':$scope.name,'add_points':$scope.add_points,'remarks':$scope.remarks}).success(function (reps){
            $scope.search(); 
            $('#man_ebank_score_add_points').modal('hide');
            alert(reps.data)
        });
    };
    $scope.del = function(row){
        if(confirm("确认删除？")){
            villageinputService.ebank_add_delete({'row_id':row[7]}).success(function(reps){
                alert(reps.data);
                $scope.search();
            })
        }else{
    ...
    $scope.to_edit = function(row){
        $('#man_ebank_score_add_points_edit_modal').modal('show');
        $scope.kyear = row[0];
        $scope.org_no = row[1];
        $scope.org_name = row[2];
        $scope.user_name = row[3];
        $scope.name = row[4];
        $scope.add_points= row[5];
        $scope.remarks = row[6];
        $scope.item_id = row[7];
    };
    $scope.edit_save = function(){
        ...
        else if(!($scope.filterNum($scope.add_points))){
            alert("附加分请输入合法数值");
            return;
        }
        else{
        ...
        villageinputService.ebank_add_update({'item_id':$scope.item_id,'kyear':$scope.kyear,'org_no':$scope.org_no,'org_name':$scope.org_name,'user_name':$scope.user_name,'name':$scope.name,'add_points':$scope.add_points,'remarks':$scope.remarks}).success(function (reps){
            $scope.search(); 
            $('#man_ebank_score_add_points_edit_modal').modal('hide');
            alert(reps.data)
        });
    };
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/villageinput/ebak_add_upload/",
            data: form,
            processData: false,
            contentType: false,
...
man_ebank_score_add_pointsController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','villageinputService','branchmanageService'];

angular.module('YSP').service('man_ebank_score_add_pointsController', man_ebank_score_add_pointsController);
