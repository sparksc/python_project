 ...
 * Manmid Controller
 */
function ManmidController($scope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_mid?export=1";
        ...
        console.log(params)
        SqsReportService.info('man_mid',params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_mid?export=1"+"&date="+moment().format('YYYYMMDD');
};

ManmidController.$inject = ['$scope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];
