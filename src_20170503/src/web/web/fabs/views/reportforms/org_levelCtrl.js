 ...
 * org_levelController
 */
function org_levelController($scope,$rootScope, $filter, SqsReportService,store,branchmanageService,depappointService,org_levelService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_level?export=1"+"&DATE_ID="+ $scope.cust_search.SYEAR + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    ...
    function do_search(){
        ...
        SqsReportService.info('org_level', params).success(function(resp) {
    ...
    $scope.calculate = function(){
        $("div[name='loading']").modal("show");
        org_levelService.calculate({}).success(function(resp){
            $scope.search();
            alert(resp.data);
            $("div[name='loading']").modal("hide");
        });                                                        
    ...
    $scope.affirm = function(){
        $("div[name='loading']").modal("show");
        org_levelService.affirm({}).success(function(resp){
            $scope.search();
            alert(resp.data);
            $("div[name='loading']").modal("hide");
       }); 
    ...
    $scope.change_level = function(row){
         $('#org_level_modal').modal('show');
         $('#org_level_new_add_button').hide();
         $('#org_level_save_edit_button').show();
         console.log(row);
         $scope.newstaffdata[0] = row[37];
         $scope.newstaffdata[1] = row[39];
    ...
    $scope.change_save = function(){
        ...
        console.log(nsd);
        org_levelService.change_save(nsd).success(function (reps){
            $scope.search();
            $('#org_level_modal').modal('hide');
            alert(reps.data)
        });
    };
...
org_levelController.$inject = ['$scope', '$rootScope','$filter', 'SqsReportService','store','branchmanageService','depappointService','org_levelService'];
angular.module('YSP').service('org_levelController', org_levelController);
