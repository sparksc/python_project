 ...
 * Mancard Controller
 */
function MancreditcardController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.find_users = function(target){
        ...
        for(i in $scope.model1){
            if($scope.model1[i].branch_code == target.ORG0_CODE){role_id=$scope.model1[i].role_id;}
        }
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_credit_card?export=1";
        ...
        console.log(params)
        SqsReportService.info('man_credit_card',params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_credit_card?export=1"+"&date="+moment().format('YYYYMMDD');
...
MancreditcardController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('MancreditcardController', MancreditcardController); 
