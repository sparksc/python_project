 ...
 */
ysp.controller("addharvestController",function ($scope,store, $filter, $rootScope, SqsReportService, addharvestService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/addharvest?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    $scope.newstaffdata= {};    
    ...
    */    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/addharvest/upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('addharvest', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
    ...
    $scope.add = function(){
        ...
        $('#addharvest_modal').modal('show');
        $('#addharvest_new_add_button').show();
        $('#addharvest_save_edit_button').hide();
    };
    ...
    $scope.add_save = function(){
       // console.log($scope.newstaffdata)
        var nsd ={};
        if($scope.newstaffdata[0] == ''||$scope.newstaffdata[1] == ''  || $scope.newstaffdata[2] == '' || $scope.newstaffdata[3] == '' || $scope.newstaffdata[4] == '' || $scope.newstaffdata[5] == '' || $scope.newstaffdata[6]=='' || $scope.newstaffdata[7]==''|| $scope.newstaffdata[8]==''){
            alert("输入不能为空");
            return;
        }
        ...
        if($scope.newstaffdata[5]<0 || $scope.newstaffdata[6]<0 || $scope.newstaffdata[7]<0){
            alert("请输入正数");
            return;
        }
        ...
        nsd["online_home"]=$scope.newstaffdata[5];
        nsd["online_buy"]=$scope.newstaffdata[6];
        nsd["online_two"]=$scope.newstaffdata[7];
        nsd["remarks"]=$scope.newstaffdata[8];
        ...
        SqsReportService.info('addharvest', params).success(function(resp) {
            ...
            if (($scope.data.rows || []).length > 0) {
                ...
                console.log(nsd)
                var nd = {"newdata":nsd};
                addharvestService.add_save(nsd).success(function (reps){
                    $scope.search(); 
                    $('#addharvest_modal').modal('hide');
                    alert(reps.data)
                });
            }
         });
    ...
    $scope.edit = function(row){
        $('#addharvest_modal').modal('show');
        $('#addharvest_new_add_button').hide();
        $('#addharvest_save_edit_button').show();
    ...
    $scope.edit_save = function(){
        ...
        nsd["online_two"]=$scope.newstaffdata[7];
        nsd["remarks"]=$scope.newstaffdata[8];
        nsd["item_id"]=$scope.newstaffdata[9];
        /*
        var nd = {"newdata":nsd};*/
        addharvestService.edit_save(nsd).success(function (reps){
             $scope.search(); 
             $('#addharvest_modal').modal('hide');
             alert(reps.data)
        });
    ...
    $scope.del = function(item){
         ...
         if(confirm("确认删除？")){
             console.log(item[13]);     
             addharvestService.del({"item_id":item[13]}).success(function (reps){
             //console.log(reps) 
                 $scope.search(); 
                 $('#addharvest_modal').modal('hide');
                 alert(reps.data)
