 ...
 * Mandep Controller
 */
function SalaryConuterController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService,StaffConutSalService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/salary_count?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code
        ...
        SqsReportService.info('salary_count',params).success(function(resp) {
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/salary_count?export=1a"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    ...
    var count_element=angular.element('#count_add_modal');
    //查找对应页面
    ...
    $scope.edit_count=function(){
        count_element.modal('show');
        $scope.connt_DATE_ID=moment();
    
    }
    $scope.count=function(){
        count_element.modal('hide');
        var nd={'count_date': $scope.connt_DATE_ID.format('YYYYMMDD')} 
        var count={'count':nd}
        $("div[name='loading']").modal("show");
        console.log(nd)
        StaffConutSalService.add_count(count).success(function (reps){
            alert(reps.data)
            $("div[name='loading']").modal("hide");
            console.log(reps.data)
            $scope.cust_search.DATE_ID=$scope.connt_DATE_ID
        }).error(function (reps){
            $("div[name='loading']").modal("hide");
        })
    }
    $scope.edit=function(row){
        $('#salary_count_edit_modal').modal('show')
        $scope.salary.now_date_id=row[0] 
        $scope.salary.org_code=row[1] 
        $scope.salary.org_name=row[2] 
        $scope.salary.sale_code=row[3] 
        $scope.salary.sale_name=row[4] 
        $scope.salary.zw=row[5] 
        $scope.salary.level=row[6] 
        $scope.salary.shiping=row[7] 
        $scope.salary.safe_flag=row[8] 
        $scope.salary.base_pay=row[9] 
        $scope.salary.position=row[10] 
        $scope.salary.safe_salary=row[11] 
        }

      $scope.edit_save=function(){
      $('#salary_count_edit_modal').modal('hide')
      var nd={'info':$scope.salary}
       StaffConutSalService.edit_save(nd).success(function (reps){
        alert(reps.data);
        $scope.search();
       })
      }
...
SalaryConuterController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService','StaffConutSalService'];

angular.module('YSP').service('SalaryConuterController', SalaryConuterController); 
