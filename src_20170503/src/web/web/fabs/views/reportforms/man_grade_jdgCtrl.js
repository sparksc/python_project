 ...
 */
function man_gradejdgController($scope,store, $filter, $rootScope, SqsReportService,man_gradejdgService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_gradejdg?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('man_gradejdg', params).success(function(resp) {
    ...
    $scope.add = function(){
        ...
        $('#man_grade_jdg').modal('show');
    };
    $scope.save = function(){
        if($scope.kyear == ''||$scope.org_no==''||$scope.org_name==''||$scope.org_name==''||$scope.user_name==''||$scope.name==''||$scope.loan_exp==''||$scope.illegal_score==''){
            alert("信息未填完全,请检查!");
            return;
        }
        //if($scope.kyear instanceof moment){
        //    $scope.kyear  = $scope.kyear.format("YYYY");
        //}
        ...
        if(!($scope.filterInt($scope.loan_exp))){
            alert("工作经验请输入合法整数");
            return;
        }
        if(!($scope.filterNum($scope.illegal_score))){
            alert("违规积分请输入合法数值");
            return;
        }

       
        man_gradejdgService.save({'kyear':$scope.kyear,'org_no':$scope.org_no,'org_name':$scope.org_name,'user_name':$scope.user_name,'name':$scope.name,'loan_exp':$scope.loan_exp,'illegal_score':$scope.illegal_score,'remarks':$scope.remarks}).success(function (reps){
            $scope.search(); 
            $('#man_grade_jdg').modal('hide');
            alert(reps.data)
        });
    };
    $scope.del = function(row){
        if(confirm("确认删除？")){
            man_gradejdgService.del({'row_id':row[8]}).success(function(reps){
                alert(reps.data);
                $scope.search();
            })
        }else{
    ...
    $scope.to_edit = function(row){
        $('#man_grade_jdg_edit_modal').modal('show');
        $scope.kyear = row[0];
        $scope.org_no = row[1];
        $scope.org_name = row[2];
        $scope.user_name = row[3];
        $scope.name = row[4];
        $scope.loan_exp = row[5];
        $scope.illegal_score = row[6];
        $scope.remarks = row[7];
        $scope.item_id = row[8];
    };
    $scope.edit_save = function(){
        ...
        else if(!($scope.filterInt($scope.loan_exp))){
            alert("信贷工作经验（年）请输入合法整数");
            return;
        }
        else if(!($scope.filterNum($scope.illegal_score))){
            alert("违规积分请输入合法数值");
            return;
        }
        else{
        ...
        man_gradejdgService.edit_save({'item_id':$scope.item_id,'kyear':$scope.kyear,'org_no':$scope.org_no,'org_name':$scope.org_name,'user_name':$scope.user_name,'name':$scope.name,'loan_exp':$scope.loan_exp,'illegal_score':$scope.illegal_score,'remarks':$scope.remarks}).success(function (reps){
            $scope.search(); 
            $('#man_grade_jdg_edit_modal').modal('hide');
            alert(reps.data)
        });
    };
...
man_gradejdgController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','man_gradejdgService','branchmanageService'];

angular.module('YSP').service('man_gradejdgController', man_gradejdgController);
