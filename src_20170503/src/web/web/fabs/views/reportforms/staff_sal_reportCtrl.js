...
function StaffSalReportController($scope, $rootScope, store,$filter,SqsReportService, permissionService,branchmanageService,StaffSalNewService) {
   ...
   $scope.upload_excel = function(){
    ...
    {   
      ...
      for(var i = 0 ; i < files.length ; ++i){
        ...
        $scope.edit = function(item){
            ...
            $scope.ebank.BASE_PAY=item[7]
            $scope.ebank.POSITION_PAY=item[8]
            $scope.ebank.BRANCH_NET_SAL=item[9]
            $scope.ebank.MANAGE_BUS_SAL=item[10]
            $scope.ebank.WORK_QUALITY_SAL=item[11]
            $scope.ebank.HIG_CIV_QUAL_SAL=item[12]
            $scope.ebank.JOB_SAT_SAL=item[13]
            $scope.ebank.DAY_DEP_COMP_PER=item[14]
            $scope.ebank.DAY_DEP_SAL=item[15]
            $scope.ebank.DAY_DEP_SEC_FEN=item[16]
            $scope.dep.LAST_AVG_SAL=item[17]
            $scope.dep.ADD_AVG_SAL=item[18]
            $scope.ebank.CREDIT_POOL=item[19]
            $scope.loan.TOTAL_NUM_SAL =item[20]
            $scope.loan.AVG_SAL=item[21]
            $scope.loan.PRI_ADD_NUM_SAL=item[22]
            $scope.loan.PUB_ADD_NUM_SAL=item[23]
            $scope.loan.ADD_AVG_ASL=item[24]
            $scope.loan.TWO_CARD_LOANRATE_SAL=item[25]
            $scope.loan.ELEC_FILE_INFO_SAL=item[26]
            $scope.ebank.INTER_SET_SAL=item[27]
            $scope.ebank.SALE_VOC_SAL=item[28]
            $scope.ebank.ADD_EFC_CURSAL=item[29]
            $scope.ebank.EPAY_ADD_NUM_SAL=item[30]
            $scope.ebank.ADD_HIGH_POS_SAL=item[31]
            $scope.ebank.ADD_LOW_POS_SAL=item[32]
            $scope.card.SALARY=item[33]
            $scope.ebank.ADD_FUNON_SAL=item[34]
            $scope.ebank.ADD_THIRD_DEPO_SAL=item[35]
            $scope.ebank.ADD_ETC_NUM_SAL=item[36]
            $scope.ebank.PER_CAR_DANERSAL=item[37]
            $scope.ebank.MB_ADD_NUM_SAL=item[38]
            $scope.ebank.CB_ADD_NUM_SAL=item[39]
            $scope.ebank.BUM_HOM_SAL=item[40]
            $scope.ebank.FARM_SERV_SAL=item[41]
            $scope.ebank.QJ_BAD_LOAN_SAL=item[42]
            $scope.ebank.OTHER_ACHI_SAL=item[43]
            $scope.ebank.COMPRE_SAL=item[44]
            $scope.ebank.LABOR_COMP_SAL=item[45]
            $scope.ebank.PROV_FUND_SAL=item[46]
            $scope.ebank.SAFE_FAN_SAL=item[47]
            $scope.ebank.ALL_RISK_SAL=item[48]
            $scope.ebank.BAD_LOAN_PERSAL=item[49]
            $scope.ebank.FTP_ACH_SAL=item[50]
            $scope.ebank.COUNT_COMPLE_SAL=item[51]
            $scope.ebank.COUNT_COP_SSAL=item[52]
            $scope.ebank.HP_FINA_SAL=item[53]
            $scope.ebank.OTHER_SPEC_SAL1=item[54]
            $scope.ebank.OTHER_SPEC_SAL2=item[55]
            $scope.ebank.OTHER_SPEC_SAL3=item[56]
            $scope.ebank.OTHER_SPEC_SAL4=item[57]
            $scope.ebank.OTHER_SPEC_SAL5=item[58]
            $scope.ebank.BRANCH_SECO_FEN1=item[59]
            $scope.ebank.BRANCH_SECO_FEN2=item[60]
            $scope.ebank.BRANCH_SECO_FEN3=item[61]
            $scope.ebank.BRANCH_SECO_FEN4=item[62]
            $scope.ebank.OTHER_ACH_WAGES=item[63]
            $scope.ebank.OVER_WORK_SAL=item[64]
            $scope.ebank.OTHER_SAL1_DUAN=item[65]
            $scope.ebank.OTHER_SAL2=item[66]
            $scope.ebank.OTHER_SAL3_WEI=item[67]
            $scope.ebank.OTHER_SAL4_KE=item[68]
            $scope.ebank.OTHER_SAL5_GE=item[69]
            $scope.ebank.OTHER_SAL6=item[70]
            $scope.ebank.OTHER_SAL7=item[71]
            $scope.ebank.OTHER_SAL8=item[72]
            LAST_AVG_SAL_flag=item[17]
            TOTAL_NUM_SAL_flag=item[20]
            AVG_SAL_flag=item[21]
       }
        ...
        $scope.edit_save = function (){
            ...
            $scope.dep.LAST_AVG_SAL=Number($scope.dep.LAST_AVG_SAL)*12
            $scope.dep.LAST_AVG_SAL=$scope.dep.LAST_AVG_SAL.toString()

            $scope.loan.TOTAL_NUM_SAL=Number($scope.loan.TOTAL_NUM_SAL)*12
            $scope.loan.TOTAL_NUM_SAL=$scope.loan.TOTAL_NUM_SAL.toString()

            $scope.loan.AVG_SAL=Number($scope.loan.AVG_SAL)*12
            $scope.loan.AVG_SAL=$scope.loan.AVG_SAL.toString()
    ...
    $scope.add_save = function (){
      ...
      $scope.dep_add.LAST_AVG_SAL=Number($scope.dep_add.LAST_AVG_SAL)*12
      $scope.dep_add.LAST_AVG_SAL=$scope.dep_add.LAST_AVG_SAL.toString()

      $scope.loan_add.TOTAL_NUM_SAL=Number($scope.loan_add.TOTAL_NUM_SAL)*12
      $scope.loan_add.TOTAL_NUM_SAL=$scope.loan_add.TOTAL_NUM_SAL.toString()

      $scope.loan_add.AVG_SAL=Number($scope.loan_add.AVG_SAL)*12
      $scope.loan_add.AVG_SAL=$scope.loan_add.AVG_SAL.toString()
