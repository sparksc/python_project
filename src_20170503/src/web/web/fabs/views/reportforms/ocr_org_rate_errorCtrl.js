 ...
 */
function ocrOrgRateErrorController($scope,store, $filter, $rootScope, SqsReportService,branchmanageService,countexam_basevolservice) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ocr_org_rate_error?export=1";
        ...
        console.log(params)
        SqsReportService.info('ocr_org_rate_error',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
        if(resp.rows[0]=='未'){
            alert("未到月末,不能查看")
    ...
    var add_element=angular.element('#ocr_org_rate_error__modal');
    var add_button_element=angular.element('#ocr_org_rate_error_new_add_button')
    var edit_button_element=angular.element('#ocr_org_rate_error_save_edit_button')
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ocr_org_rate_error?export=1"+"&DATE_ID="+moment().format('YYYYMMDD');
    ...
    $scope.save = function(){
        ...
        console.log(nsd)
        var nd = {"newdata":nsd};
        countexam_basevolservice.ocr_org_rate_error_save(nd).success(function (reps){
    ...
    $scope.edit_save = function(){
        ...
        var nd = {"newdata":nsd};
        countexam_basevolservice.ocr_org_rate_error_update(nd).success(function (reps){
    ...
    nsd["id"]=row[8];
    var nd = {"newdata":nsd};
        countexam_basevolservice.ocr_org_rate_error_delete(nd).success(function (reps){
        //console.log(reps) 
        $scope.search(); 
        add_element.modal('hide');
        });
    ...
    if(files.length==0)
        ...
        $.ajax({
        type: "POST",
        url : base_url+"/countexam_basevol_hander/ocr_org_rate_error_upload",
        data: form,
        processData: false,
        contentType: false,
...
ocrOrgRateErrorController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branchmanageService','countexam_basevolservice'];

angular.module('YSP').service('ocrOrgRateErrorController', ocrOrgRateErrorController);
