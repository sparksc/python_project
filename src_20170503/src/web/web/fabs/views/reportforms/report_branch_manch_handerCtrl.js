 ...
 */
function reportBranchManchHanderController($scope,store, $filter, $rootScope, SqsReportService,branchmanageService,hallManagerHanderServer) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/report_branch_manch_hander?export=1";
        ...
        console.log(params)
        SqsReportService.info('report_branch_manch_hander',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
        if(resp.rows[0]=='未'){
            alert("未到季末,不能查看")
    ...
    var add_element=angular.element('#report_branch_manch_hander__modal');
    var add_button_element=angular.element('#report_branch_manch_hander_new_add_button')
    var edit_button_element=angular.element('#report_branch_manch_hander_save_edit_button')
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/report_branch_manch_hander?export=1"+"&DATE_ID="+moment().format('YYYYMMDD');
    ...
    $scope.save = function(){
        ...
        if (Number($scope.newstaffdata[4])<0){
            $scope.newstaffdata[4]=null
            alert("未正常运行天数不能为负数")
            return
        }
        if (Number($scope.newstaffdata[4]).toString()=='NaN'){
            $scope.newstaffdata[4]=null
            alert("未正常运行天数存在非法字符")
            return
        }
        ...
        nsd["atm_no"]=$scope.newstaffdata[3];
        nsd["not_work_days"]=$scope.newstaffdata[4];
        console.log(nsd)
        var nd = {"newdata":nsd};
        hallManagerHanderServer.branch_atm_save(nd).success(function (reps){
    ...
    $scope.edit_save = function(){
        ...
        var nd = {"newdata":nsd};
        hallManagerHanderServer.branch_atm_update(nd).success(function (reps){
    ...
    if(files.length==0)
        ...
        $.ajax({
        type: "POST",
        url : base_url+"/hall_manager_hander/branch_atm_upload/",
        data: form,
        processData: false,
        contentType: false,
...
reportBranchManchHanderController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branchmanageService','hallManagerHanderServer'];

angular.module('YSP').service('reportBranchManchHanderController', reportBranchManchHanderController);
