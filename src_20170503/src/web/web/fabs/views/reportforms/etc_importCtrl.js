/**
 * depositplanparameter Controller
 */
function etc_importController($scope,store, $filter, $rootScope, SqsReportService,villageinputService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/etc_import?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    $scope.newstaffdata= {};    
    $scope.change_name = function(target){
        if(target == null){
            $scope.etc_update.org_no = '';
            $scope.etc_update.teller_no = '';
       }
    ...
    $scope.change_name2 = function(target){
       ...
       for(var i in $scope.model3){
            if($scope.model3[i].user_name==target){
                $scope.teller_name = $scope.model3[i].name;
            }
       } 
    }
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/villageinput/etc_data_upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.upload_excel_org = function(){
        var files = angular.element(document.getElementById('tab_'+ $scope.tabId + '_content')).find("#elec_arch").prop('files');
        if(files.length==0)
        {
            alert("请先选择对应的文件内容,再导入!");
        ...
        form.append('application_id',$scope.applicationId)
        form.append('about','elec_arch')
        form.append('login_branch_no',$rootScope.user_session.branch_code)
        $.ajax({
            type: "POST",
            url : base_url+"/villageinput/etc_org_upload/",
            data: form,
            processData: false,
            contentType: false,
            beforeSend: function(request) {
    ...
    $scope.search = function() {
        ...
        $scope.cur_page = 1;
        $scope.total_count = 0;
        if(($scope.cust_search.DATE_ID || []).length > 0){
            if(!$scope.filterInt($scope.cust_search.DATE_ID)){
                alert("请输入合法年份，如20170101");
                $("div[name='loading']").modal("hide");
                return;
            }
        }
        ...
        for(var key in $scope.cust_search){
            ...
            if(key=='DATE_ID')
                params['DATE_ID'] = params['DATE_ID'];
            $scope.excelurl = $scope.excelurl + "&"+key+"="+params[key]
            }
        }
        SqsReportService.info('etc_import', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
    ...
    $scope.del = function(item){
		...
		if(r==true){
            ...
            nsd["id"] = item[9];
            var nd = {"newdata":nsd};
           villageinputService.etc_data_delete(nd).success(function(resp){
           alert(resp.data);
           $scope.search();
           });
        }
    ...
    //date_id, cust_net_no, cust_name, card_no, card_name, sign_org_no, sign_teller_no, org_no, teller_no
    $scope.edit = function(item){
        $('#etc_import_modal').modal('show');
        $scope.etc_update={};
        $scope.etc_update.id = item[9]
        //a = String(item[1]);
        //str = a.substr(0,4)+'-'+a.substr(4,2)+'-'+a.substr(6,2);
        //$scope.etc_update.date_id = moment(str);
        $scope.etc_update.date_id = item[0]
        $scope.etc_update.cust_net_no = item[1]
        $scope.etc_update.cust_name = item[2]
        $scope.etc_update.card_no = item[3]
        $scope.etc_update.card_name = item[4]
        $scope.etc_update.sign_org_no = item[5]
        $scope.etc_update.sign_teller_no = item[6]
        $scope.etc_update.org_no = item[8]
        $scope.etc_update.teller_no = item[7]
        $scope.etc_update.login_branch_no= $rootScope.user_session.branch_code
    };
    $scope.edit_save = function (){
        $('#etc_import_modal').modal('hide');
        villageinputService.etc_data_edit({'etc_update':$scope.etc_update}).success(function(resp){
            alert(resp.data);
            $scope.search();
            $('#etc_import_modal').modal('hide');
        });
...
etc_importController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','villageinputService','branchmanageService'];

angular.module('YSP').service('etc_importController', etc_importController);
