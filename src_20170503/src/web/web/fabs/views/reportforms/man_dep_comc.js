/**
 * mandepcom Controller
 */
function mandepcomController($scope,$rootScope, $filter, SqsReportService,monsalaryService,store,branchmanageService,depappointService,dictdataService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    function do_search(){
        ...
        SqsReportService.info('mandepcom', params).success(function(resp) {
            $scope.data = resp;
...
mandepcomController.$inject = ['$scope','$rootScope', '$filter', 'SqsReportService','monsalaryService','store','branchmanageService','depappointService','dictdataService'];

angular.module('YSP').service('mandepcomController', mandepcomController);
