/**
 */
function OrgintController($scope, $rootScope, $filter, SqsReportService,store,branchmanageService,depappointService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    function do_search(){
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_int?export=1a"; 
        ...
        SqsReportService.info('org_int', params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_int?export=1a"+"&tdate="+moment().format('YYYYMMDD');
...
OrgintController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','store','branchmanageService','depappointService'];
angular.module('YSP').service('OrgintController', OrgintController);
