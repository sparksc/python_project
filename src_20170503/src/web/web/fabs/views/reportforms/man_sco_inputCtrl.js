/**
 * manscoinput Controller
 */
function manscoinputController($scope,store, $filter, $rootScope, SqsReportService,manscoinputService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_sco_input?export=1"
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('man_sco_input', params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.add = function(){
    $scope.newstaffdata={};
    $('#manscoinput_modal').modal('show');
    $('#manscoinput_new_add_button').show();
    $('#manscoinput_save_edit_button').hide();
    };
    ...
    $scope.save = function(){
        ...
        $scope.search(); 
        $('#manscoinput_modal').modal('hide');
        });
    };
    $scope.to_edit = function(row){
    $('#manscoinput_modal').modal('show');
    $('#manscoinput_new_add_button').hide();
    $('#manscoinput_save_edit_button').show();
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/manscoinput/upload/",
            data: form,
            processData: false,
            contentType: false,
...
manscoinputController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','manscoinputService','branchmanageService'];

angular.module('YSP').service('manscoinputController', manscoinputController);
