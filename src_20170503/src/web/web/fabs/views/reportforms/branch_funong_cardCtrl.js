 ...
 */
function branch_funong_cardController($scope,store, $filter, $rootScope, SqsReportService,branch_funong_cardService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/branch_funong_card?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('branch_funong_card', params).success(function(resp) {
    ...
    $scope.add = function(){
        ...
        $('#branch_funong_card').modal('show');
    };
    $scope.save = function(){
        if($scope.kyear == ''||$scope.org_no==''||$scope.org_name==''||$scope.target==''){
            alert("信息未填完全,请检查!");
            return;
        }
        ...
        if(!$scope.filterNum($scope.target)){
            alert("请输入合法数值");
            return;
        }

       
        branch_funong_cardService.count_save({'kyear':$scope.kyear,'org_no':$scope.org_no,'org_name':$scope.org_name,'target':$scope.target,'remarks':$scope.remarks}).success(function (reps){
            $scope.search(); 
            $('#branch_funong_card').modal('hide');
            alert(reps.data)
        });
    };
    $scope.del = function(row){
        if(confirm("确认删除？")){
            branch_funong_cardService.conunt_del({'row_id':row[5]}).success(function(reps){
                alert(reps.data);
                $scope.search();
            })
        }else{
    ...
    $scope.to_edit = function(row){
        $('#branch_funong_card_edit_modal').modal('show');
        $scope.kyear = row[0];
        $scope.org_no = row[1];
    ...
    $scope.edit_save = function(){
        ...
        branch_funong_cardService.count_edit_save({'item_id':$scope.item_id,'kyear':$scope.kyear,'org_no':$scope.org_no,'org_name':$scope.org_name,'target':$scope.target,'remarks':$scope.remarks}).success(function (reps){
            $scope.search(); 
            $('#branch_funong_card_edit_modal').modal('hide');
            alert(reps.data)
        });
    };
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/branch_funong_card/count_upload/",
            data: form,
            processData: false,
            contentType: false,
...
branch_funong_cardController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','branch_funong_cardService','branchmanageService'];

angular.module('YSP').service('branch_funong_cardController', branch_funong_cardController);
