 ...
 * ManLoan Controller
 */
function ManLoanTimesController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/man_loan_times?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") +"&END_DATE_ID="+ $scope.cust_search.END_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code; 
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('man_loan_times',params).success(function(resp) {
...
ManLoanTimesController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('ManLoanTimesController', ManLoanTimesController); 
