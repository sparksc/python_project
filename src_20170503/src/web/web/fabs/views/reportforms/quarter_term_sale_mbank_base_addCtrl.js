 ...
 */
function quartertermsalembankbaseaddController($scope,store, $filter, $rootScope, SqsReportService,villageinputService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank_base_add?export=1"
    $scope.newstaffdata= {};    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/villageinput/quarter_term_sale_mbank_base_upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        if(params["DATE_ID"])
        {
            if (params["DATE_ID"].length != 8)
                params.date = params["DATE_ID"].format('YYYYMMDD');
            else
                params.date = params["DATE_ID"]
        }

        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank_base_add?export=1"+"&DATE_ID="+ params.date + "&login_branch_no=" + $rootScope.user_session.branch_code

        for(var key in $scope.cust_search){
            if($scope.cust_search[key]!=''&&$scope.cust_search[key]!=null){
            params[key]=$scope.cust_search[key];
            if(key=='DATE_ID')
                params['DATE_ID'] = params.date;
            $scope.excelurl = $scope.excelurl + "&"+key+"="+params[key]
            }
        }
        SqsReportService.info('quarter_term_sale_mbank_base_add', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank_base_add?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code
...
quartertermsalembankbaseaddController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','villageinputService','branchmanageService'];

angular.module('YSP').service('quartertermsalembankbaseaddController', quartertermsalembankbaseaddController);
