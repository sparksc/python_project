 ...
 * Orgdep Controller
 */
function OrgdepController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_dep?export=1";
        ...
        SqsReportService.info('org_dep',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/org_dep?export=1"+"&date="+moment().format('YYYYMMDD')+"&org="+$rootScope.user_session.branch_code;
...
OrgdepController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('OrgdepController', OrgdepController); 
