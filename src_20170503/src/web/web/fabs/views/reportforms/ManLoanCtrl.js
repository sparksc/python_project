...
function ManLoanController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ManLoan?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('ManLoan',params).success(function(resp) {
            $scope.data = resp;
         console.log(resp)
