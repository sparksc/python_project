/**
 * ebankplanparameter Controller
 */
ysp.controller("user_extrascoreController",function ($scope,store, $filter, $rootScope, SqsReportService,user_extrascoreService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/user_extrascore?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    $scope.newstaffdata= {};    
    ...
    function find_credentials(){
        var temp = [].
            ...
            i = 0,
            j = 0;
        user_extrascoreService.credentials({'':null}).success(function(resp){
            t = resp.data
            for(i;i<t.length;i++){
                if(i == 1) continue;
                temp[j++] = t[i];
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/user_extrascore/upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('user_extrascore', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
            console.log(resp);
    ...
    $scope.add = function(){
        ...
        $('#user_extra_score_modal').modal('show');
        $('#user_extra_score_new_add_button').show();
        $('#user_extra_score_save_edit_button').hide();
    };
    ...
    $scope.add_save = function(){
       // console.log($scope.newstaffdata)
        ...
        nsd["credential_code"]=$scope.newstaffdata[5];
        nsd["credential_name"]=$scope.newstaffdata[6];
        nsd["remarks"]=$scope.newstaffdata[7];
        var org_code = $scope.newstaffdata[1] ,
            user_code = $scope.newstaffdata[3],
            credential_code = $scope.newstaffdata[5];
        params = {'ORG_CODE':org_code,'USER_CODE':user_code,'CREDENTIAL_CODE':credential_code};
        //console.log(params)
        SqsReportService.info('user_extrascore', params).success(function(resp) {
            $scope.data = resp;
            ...
            if (($scope.data.rows || []).length > 0) {
                ...
                $scope.search()
                alert("该员工已有该证书");
                $('#user_extra_score_modal').modal('hide');
            }else{
                $scope.tableMessage = ""
                console.log(nsd)
                var nd = {"newdata":nsd};
                user_extrascoreService.add_save(nsd).success(function (reps){
                    $scope.search(); 
                    $('#user_extra_score_modal').modal('hide');
                    alert(reps.data)
                });
    ...
    $scope.edit = function(row){
        $('#user_extra_score_edit_modal').modal('show');
        $('#user_extra_score_new_add_button').hide();
        $('#user_extra_score_save_edit_button').show();
    ...
    $scope.edit_save = function(){
        ...
        nsd["credential_name"]=$scope.newstaffdata[6];
        nsd["remarks"]=$scope.newstaffdata[7];
        nsd["item_id"]=$scope.newstaffdata[8];
        user_extrascoreService.edit_save(nsd).success(function (reps){
             $scope.search(); 
             $('#user_extra_score_edit_modal').modal('hide');
             alert(reps.data)
        });
    ...
    $scope.del = function(item){
         if(confirm("确认删除？")){
             console.log(item[9]);     
             user_extrascoreService.del({"item_id":item[8]}).success(function (reps){
                 $scope.search(); 
                 $('#user_extra_score_modal').modal('hide');
                 alert(reps.data)
             });
