/**
 * user_levelController
 */
ysp.controller("user_levelController",function ($scope,store, $filter, $rootScope, SqsReportService,user_levelService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/user_level?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    $scope.newstaffdata= {};    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/user_level/upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('user_level', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
    ...
    $scope.add = function(){
        ...
        $('#user_level_modal').modal('show');
        $('#user_level_new_add_button').show();
        $('#user_level_save_edit_button').hide();
    };

    $scope.add_save = function(){
        console.log($scope.newstaffdata);
        var nsd ={};
        if($scope.newstaffdata[0] == ''||$scope.newstaffdata[1] == ''  || $scope.newstaffdata[2] == '' || $scope.newstaffdata[3] == '' || $scope.newstaffdata[4] == '' || $scope.newstaffdata[5] == '' || $scope.newstaffdata[6]=='' || $scope.newstaffdata[7]=='' || $scope.newstaffdata[8] == '' ||$scope.newstaffdata[9] == '' || $scope.newstaffdata[10] == ''||$scope.newstaffdata[11] == ''){
            alert("输入不能为空");
            return;
        }
        ...
        if(!($scope.filterInt($scope.newstaffdata[9]) && $scope.filterInt($scope.newstaffdata[10]))){
            alert("行龄或临柜经验请输入合法整数");
            return;
        }
        if(!($scope.filterNum($scope.newstaffdata[6]) && $scope.filterNum($scope.newstaffdata[7]) && $scope.filterNum($scope.newstaffdata[8]) && $scope.filterNum($scope.newstaffdata[11]))){
            alert("请输入合法数字");
            return;
        }
        console.log($scope.newstaffdata)
        ...
        nsd["task_level"]=$scope.newstaffdata[5];
        nsd["civilized_service"]=$scope.newstaffdata[6];
        nsd["task_errorrate"]=$scope.newstaffdata[7];
        nsd["cust_satisfaction"]=$scope.newstaffdata[8];
        nsd["work_year"]=$scope.newstaffdata[9];
        nsd["bankterm_year"]=$scope.newstaffdata[10];
        nsd["violation_score"]=$scope.newstaffdata[11];
        nsd["remarks"]=$scope.newstaffdata[12];

        var org_code = $scope.newstaffdata[1] ,
            user_code = $scope.newstaffdata[3],
            syear = $scope.newstaffdata[0];
        params = {'ORG_CODE':org_code,'USER_CODE':user_code,'SYEAR':syear};
        SqsReportService.info('user_level', params).success(function(resp) {
            $scope.data = resp;
            ...
            if (($scope.data.rows || []).length > 0) {
                ...
                console.log(nsd)
                var nd = {"newdata":nsd};
                user_levelService.add_save(nsd).success(function (reps){
                    $scope.search(); 
                    $('#user_level_modal').modal('hide');
                    alert(reps.data)
                });
    ...
    $scope.edit = function(row){
        $('#user_level_edit_modal').modal('show');
        $('#user_level_new_add_button').hide();
        $('#user_level_save_edit_button').show();
    ...
    $scope.edit_save = function(){
        ...
        nsd["violation_score"]=$scope.newstaffdata[11];
        nsd["remarks"]=$scope.newstaffdata[12];
        nsd["item_id"]=$scope.newstaffdata[13];
        user_levelService.edit_save(nsd).success(function (reps){
             $scope.search(); 
             $('#user_level_edit_modal').modal('hide');
             alert(reps.data)
        });
    ...
    $scope.del = function(item){
         if(confirm("确认删除？")){
             user_levelService.del({"item_id":item[13]}).success(function (reps){
                 $scope.search(); 
                 $('#user_level_modal').modal('hide');
                 alert(reps.data)
             });
