 ...
 */
ysp.controller("account_rankController",function ($scope,store, $filter, $rootScope, SqsReportService, account_rankService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/account_rank?export=1"+ "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code;
    $scope.newstaffdata= {};    
    ...
    */    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/account_rank/upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('account_rank', params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.data = resp;
    ...
    $scope.add = function(){
        ...
        $('#account_rank_modal').modal('show');
        $('#account_rank_new_add_button').show();
        $('#account_rank_save_edit_button').hide();
    };
    ...
    $scope.add_save = function(){
       // console.log($scope.newstaffdata)
        ...
        if(!$scope.filterInt($scope.newstaffdata[5])){
            alert("业务差错率排名请输入数字");
            return;
        }
        if(!$scope.filterInt($scope.newstaffdata[7])){
            alert("助理会计行龄请输入数字");
            return;
        }
        if(!$scope.filterInt($scope.newstaffdata[8])){
            alert("内勤人均违规积分分值排名请输入数字");
            return;
        }
        if(!$scope.filterInt($scope.newstaffdata[9])){
            alert("电子银行替代率完成率排名请输入数字");
            return;
        }
        if(!$scope.filterInt($scope.newstaffdata[11])){
            alert("助理会计工作经验请输入数字");
            return;
        }
        ...
        nsd["repleace_rank"]=$scope.newstaffdata[9];
        nsd["skill"]=$scope.newstaffdata[10];
        nsd["experience"]=$scope.newstaffdata[11];
        ...
        SqsReportService.info('account_rank', params).success(function(resp) {
            ...
            if (($scope.data.rows || []).length > 0) {
                $scope.tableMessage = "";
                $scope.search()
                $('#account_rank_modal').modal('hide');
                alert("该员工已存在");
            }else{
                $scope.tableMessage = ""
                console.log(nsd)
                var nd = {"newdata":nsd};
                account_rankService.add_save(nsd).success(function (reps){
                    $scope.search(); 
                    $('#account_rank_modal').modal('hide');
                    alert(reps.data)
                });
    ...
    $scope.edit = function(row){
        $('#account_rank_edit_modal').modal('show');
        $('#account_rank_new_add_button').hide();
        $('#account_rank_save_edit_button').show();
    ...
    $scope.edit_save = function(){
        ...
        /*
        var nd = {"newdata":nsd};*/
        account_rankService.edit_save(nsd).success(function (reps){
             $scope.search(); 
             $('#account_rank_edit_modal').modal('hide');
             alert(reps.data)
        });
    ...
    $scope.del = function(item){
         ...
         if(confirm("确认删除？")){
             console.log(item[13]);     
             account_rankService.del({"item_id":item[13]}).success(function (reps){
             //console.log(reps) 
                 $scope.search(); 
                 $('#account_rank_modal').modal('hide');
                 alert(reps.data)
