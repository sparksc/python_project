 ...
 * Mandep Controller
 */
function teller_trans_countController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
...
//    $scope.cust_search.END_DATE_ID = moment();
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/teller_trans_count?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code
        ...
        SqsReportService.info('teller_trans_count',params).success(function(resp) {
...
teller_trans_countController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('teller_trans_countController', teller_trans_countController); 
