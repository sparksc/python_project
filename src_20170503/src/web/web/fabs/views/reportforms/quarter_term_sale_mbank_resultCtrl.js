 ...
 * Mandep Controller
 */
function quarter_term_sale_mbank_resultController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank_result?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") +"&CHECK_DATE_ID=" + $scope.cust_search.CHECK_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code
        ...
        SqsReportService.info('quarter_term_sale_mbank_result',params).success(function(resp) {
...
quarter_term_sale_mbank_resultController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('quarter_term_sale_mbank_resultController', quarter_term_sale_mbank_resultController); 
