 ...
 * Mandep Controller
 */
function quarter_term_sale_mbank_baseController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank_base?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") + "&END_DATE_ID="+ $scope.cust_search.END_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code
        for(var key in $scope.cust_search){
            if($scope.cust_search[key]!=''&&$scope.cust_search[key]!=null){
            params[key]=$scope.cust_search[key];
            if(key=='FROM_DATE_ID')params['FROM_DATE_ID']=params['FROM_DATE_ID'].format("YYYYMMDD");
            if(key=='END_DATE_ID')params['END_DATE_ID']=params['END_DATE_ID'].format("YYYYMMDD");
            $scope.excelurl = $scope.excelurl + "&"+key+"="+params[key]
            }
        }

        SqsReportService.info('quarter_term_sale_mbank_base',params).success(function(resp) {
...
quarter_term_sale_mbank_baseController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('quarter_term_sale_mbank_baseController', quarter_term_sale_mbank_baseController); 
