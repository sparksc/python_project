 ...
 * Mandep Controller
 */
function quarter_term_sale_mbankController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/quarter_term_sale_mbank?export=1"+"&FROM_DATE_ID="+ $scope.cust_search.FROM_DATE_ID.format("YYYYMMDD") + "&END_DATE_ID="+ $scope.cust_search.END_DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code
        ...
        SqsReportService.info('quarter_term_sale_mbank',params).success(function(resp) {
...
quarter_term_sale_mbankController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('quarter_term_sale_mbankController', quarter_term_sale_mbankController); 
