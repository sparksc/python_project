 ...
 */
function depositplanparameterController($scope,store, $filter, $rootScope, SqsReportService,villageinputService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/deposit_plan_parameter?export=1"
    $scope.newstaffdata= {};    
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/villageinput/deposit_upload/",
            data: form,
            processData: false,
            contentType: false,
    ...
    $scope.search = function() {
        ...
        $scope.cur_page = 1;
        $scope.total_count = 0;
        SqsReportService.info('deposit_plan_parameter', params).success(function(resp) {
            $scope.data = resp;
    ...
    $scope.show_flag = 1
    $scope.newstaffdata={};
    $('#deposit_plan_parameter_modal').modal('show');
    $('#deposit_plan_parameter_new_add_button').show();
    $('#deposit_plan_parameter_save_edit_button').hide();
    };
    ...
    $scope.save = function(){
        ...
        if( $scope.newstaffdata[6] < 0){
            alert("目标任务不能为负")
        
        }


        nsd["dbyear"]=$scope.newstaffdata[0];
        nsd["third_org_code"]=$scope.newstaffdata[1];
        nsd["third_org_name"]=$scope.newstaffdata[2];
        nsd["manager_code"]=$scope.newstaffdata[3];
        nsd["manager_name"]=$scope.newstaffdata[4];
        nsd["base"]=$scope.newstaffdata[5];
        nsd["target"]=$scope.newstaffdata[6];
        console.log(nsd)
        var nd = {"newdata":nsd};
        villageinputService.deposit_save(nd).success(function (reps){
        alert(reps.data)
        $scope.search(); 
        $('#deposit_plan_parameter_modal').modal('hide');
        });
    };
    ...
    $scope.show_flag=2
    $('#deposit_plan_parameter_modal').modal('show');
    $('#deposit_plan_parameter_new_add_button').hide();
    $('#deposit_plan_parameter_save_edit_button').show();
    ...
    $scope.edit_save = function(){
       ...
       if( $scope.newstaffdata[5]==null ||$scope.newstaffdata[5]==""){
        ...
        if( parseInt($scope.newstaffdata[6]) < 0){
            alert("目标任务不能为负")
            return 
        }

        //去掉数字的考核基数和目标任务','字符
        if($scope.newstaffdata[5].indexOf(",") != -1){
            nsd["base"] = $scope.newstaffdata[5].replace(new RegExp(',', 'gm'),''); 
        }
        else{
            nsd["base"] = $scope.newstaffdata[5];
        
        }
        if ( $scope.newstaffdata[6].indexOf(",") != -1){
            nsd["target"]=$scope.newstaffdata[6].replace(new RegExp(',', 'gm'),'');
        }
        else{
            nsd["target"]=$scope.newstaffdata[6]
        }
        if( typeof $scope.newstaffdata[0] == 'number' ){
            nsd["dbyear"]=$scope.newstaffdata[0];
        }
        else{
            nsd["dbyear"]=$scope.newstaffdata[0].format('YYYY');
        }
        nsd["third_org_code"]=$scope.newstaffdata[1];
        nsd["third_org_name"]=$scope.newstaffdata[2];
        nsd["manager_code"]=$scope.newstaffdata[3];
        ...
        var nd = {"newdata":nsd};
        villageinputService.deposit_update(nd).success(function (reps){
        $scope.search(); 
        $('#deposit_plan_parameter_modal').modal('hide');
    ...
    nsd["id"]=row[7];
    var nd = {"newdata":nsd};
        villageinputService.deposit_delete(nd).success(function (reps){
        //console.log(reps) 
        $scope.search(); 
        $('#deposit_plan_parameter_modal').modal('hide');
        });
...
depositplanparameterController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','villageinputService','branchmanageService'];

angular.module('YSP').service('depositplanparameterController', depositplanparameterController);
