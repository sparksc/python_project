ysp.controller('personCreditController', function($scope, $rootScope, creditService, CustomerSearchService, PledgeService ){
    ...
    $scope.custSearchConfirm = function(){
        ...
        var cust_state = $scope.chosenCust.state;
        if (cust_state=='完善'){
        var cust_id = $scope.chosenCust.id;
        var cust_name = $scope.chosenCust.name;
        var tabName = cust_name+'的贷款申请';
        var htmlContent = '<div ng-include="'+'\'views/credit/personCredit/personCreditInfo.html' +'\'" ></div>';
        var eventObj = { 'create':'', 'focus':'', 'loseFocus':'', 'close':'',};
        var tab_id = $rootScope.addTab(tabName, htmlContent, eventObj, true, {'customer':$scope.chosenCust});
        }else{alert('用户信息不完善,请完善后再做申请')}
    };
...
ysp.controller('personApplicationController', function($scope, store,$rootScope, creditService, CustomerSearchService){
     $scope.pre_apply={};
    ...
    $scope.init_business_type = function(){
        ...
        var setting = {};
        var Nodes=[];
        creditService.products('个人业务').success(function(resp){
            var data = resp.data;
            show_product(data)
