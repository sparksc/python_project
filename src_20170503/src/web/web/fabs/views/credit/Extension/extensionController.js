ysp.controller('extensionController', function($scope, $rootScope, loanService, ExtensionService){
    /* Query fiveLevel */
    $scope.tableHead=['申请编号',  '客户编号','客户名称', '交易ID', '审批状态', '操作'];
    $scope.tableMessage='请点击查询';
    $scope.queryCond = {};
    $scope.tableData = {};
    $scope.queryExtension = function(){
        ExtensionService.query().success(function(resp){
    ...
    $scope.onDetail = function(d){
        var htmlContent = '<div ng-include="'+'\'views/credit/Extension/info.html' +'\'" ></div>';
        var eventObj = { 'create':'', 'focus':'', 'loseFocus':'', 'close':'',};
        var cust_name = d.party.name
        var tabName = cust_name+'的贷款展期详情';
        var tab_id = $rootScope.addTab(tabName, htmlContent, eventObj, true, {'data':d} );
    };
    $scope.onDelete = function(id){
        if (confirm("确定撤销分类申请吗？")) {
            ExtensionService.deleteById(id).then(function(){
                $scope.queryExtension();
            })
        }
    };
...
ysp.controller('extensionBaseController', function($scope, $compile, loanService,userService,ExtensionService,approvalService){
    $scope.edit_flag = true;
    $scope.subTabIndex = 0;
    $scope.currIndex = 0;
    $scope.subTab = {}; //check if open the same tab
    if($scope.application_id){
        ExtensionService.query_by_id($scope.application_id).success(function(resp){
    ...
    $scope.onSubmit = function(){
        if ($scope.data.extension.id){
            ExtensionService.submit($scope.data).success(function(resp){
                alert("申请已提交!")
                $scope.edit_flag = true;
            })
        }else{
            ExtensionService.save_submit($scope.data).success(function(resp){
                alert("申请已提交!")
                $scope.edit_flag = true;
                $scope.data.extension=resp.data.extension
    ...
    $scope.onSave = function(){
        if ($scope.data.extension.id){
            ExtensionService.update($scope.data).then(function(resp){
            ...
            ExtensionService.save($scope.data).success(function(resp){
                $scope.data= resp.data;
