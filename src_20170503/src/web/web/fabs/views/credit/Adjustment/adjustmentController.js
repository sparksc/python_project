ysp.controller('adjustmentController', function($scope, $rootScope, loanService, AdjustmentService){
    /* Query fiveLevel */
    $scope.tableHead=['申请编号',  '客户编号','客户名称', '交易ID', '审批状态', '操作'];
    $scope.tableMessage='请点击查询';
    $scope.queryCond = {};
    $scope.tableData = {};
    $scope.queryAdjustment = function(){
        AdjustmentService.query().success(function(resp){
    ...
    $scope.onDetail = function(d){
        var htmlContent = '<div ng-include="'+'\'views/credit/Adjustment/info.html' +'\'" ></div>';
    ...
    $scope.onDelete = function(id){
        if (confirm("确定撤销分类申请吗？")) {
            AdjustmentService.deleteById(id).then(function(){
                $scope.queryAdjustment();
            })
        }
    };
...
ysp.controller('adjustmentBaseController', function($scope, $compile, loanService,userService,AdjustmentService,approvalService){
    $scope.edit_flag = true;
    $scope.subTabIndex = 0;
    $scope.currIndex = 0;
    $scope.subTab = {}; //check if open the same tab
    if($scope.application_id){
        AdjustmentService.query_by_id($scope.application_id).success(function(resp){
    ...
    $scope.onSubmit = function(){
        if ($scope.data.adjustment.id){
            AdjustmentService.submit($scope.data).success(function(resp){
                alert("申请已提交!")
                $scope.edit_flag = true;
            })
        }else{
            AdjustmentService.save_submit($scope.data).success(function(resp){
                alert("申请已提交!")
                $scope.edit_flag = true;
                $scope.data.adjustment=resp.data.adjustment
    ...
    $scope.onSave = function(){
        if ($scope.data.adjustment.id){
            AdjustmentService.update($scope.data).then(function(resp){
            ...
            AdjustmentService.save($scope.data).success(function(resp){
                $scope.data= resp.data;
