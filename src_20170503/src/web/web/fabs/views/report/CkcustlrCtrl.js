 ...
 * ckcustlr Controller
 */
function ckcustlrController($scope,store, $filter, $rootScope, SqsReportService,custhkService,dictdataService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/custhk1?export=1&TYP="+encodeURI("存款");
    $scope.newstaffdata= {};
    ...
    $scope.add = function(){
        ...
        $scope.add_zh = '';
        $('#check_ckcustlr').show();
        $('#ckcustlr_modal').modal('show');
        $('#Ckcustlr_input').hide();
        $('#Ckcustlr_edit').hide();
        $('#ckcustlr_new_add_button').hide();
        $('#ckcustlr_save_edit_button').hide();
        $scope.tableMessage1 = "请输入客户号";
    };
    $scope.check = function(account){
        ...
        if (($scope.newdata.rows || []).length > 0) {
                $scope.tableMessage1 = "";
            SqsReportService.info('custcheck',{'cust_no':account,'typ':"存款",'org_no':$rootScope.user_session.branch_code}).success(function(resp) {
               $scope.newstaffdata = resp;
                    if ((resp.rows || []).length > 0) {
                        SqsReportService.info('custcheck_manager',{'cust_no':account,'typ':"存款",'org_no':$rootScope.user_session.branch_code}).success(function(resp){
                        ...
                        $scope.newstaffdata[3] = '是';

                        $('#ckcustlr_new_add_button').show();
                    }
            });
            } else {
    ...
    $scope.save = function(){
        ...
        if(per>100){
            ...
            alert('每个对公客户只能有一个主办客户经理,请检查!');
            return;
            }
        else {
        custhkService.check_manager({'staff':staff,'org_no':$rootScope.user_session.branch_code,'cust_no':$scope.newdata.rows[0][0],'typ':'存款'}).success(function (reps){
            if(reps.data=='0'){
                alert('请输入本网点员工!')
                return;
                }
                ...
                custhkService.ssave({'newdata':nnsd}).success(function (reps){     //custhook挂钩
                        $('#ckcustlr_modal').modal('hide');
                        $scope.search(); 
                        alert('录入成功！');
                    });
                }
    ...
    $scope.to_edit = function(row){
        ...
        $('#ckcustlr_modal').modal('show');
        $('#ckcustlr_new_add_button').hide();
        $('#ckcustlr_save_edit_button').show();
        $('#update_per').show();
        if(row[4]=='对私'){
            $('#update_per').hide();
        }
    ...
    $scope.edit_save = function(){
        ...
        custhkService.check_manager({'staff':[$scope.newstaffdata[0]],'org_no':$rootScope.user_session.branch_code,'cust_no':$scope.newdata.rows[0][0],'typ':'存款'}).success(function (reps){
            if(reps.data=='0'){
                alert('请输入本网点员工!')
                return;
            }
...
ckcustlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','custhkService','dictdataService'];

angular.module('YSP').service('ckcustlrController', ckcustlrController);
