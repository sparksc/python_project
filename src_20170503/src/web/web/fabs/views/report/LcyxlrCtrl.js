 ...
 * lcyxlr Controller
 */
function lcyxlrController($scope,store, $filter, $rootScope, SqsReportService,accthkService,dictdataService,custhkService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.add = function(){
        ...
        $scope.add_zh = '';
        $('#check_lcyxlr').show();
        $('#lcyxlr_modal').modal('show');
        $('#Lcyxlr_input').hide();
        $('#Lcyxlr_edit').hide();
        $('#lcyxlr_new_add_button').hide();
        $('#lcyxlr_save_edit_button').hide();
    ...
    $scope.check = function(account){
        if(account == null||account == ''){
            $scope.tableMessage1="流水号不能为空"
        }
        else{
        SqsReportService.info('fin_accthk',{'ACCOUNT_NO':account}).success(function(resp){
        $scope.newdata=resp;
        if (($scope.newdata.rows || []).length > 0) {
            ...
            if(resp.rows[0][4]!=$rootScope.user_session.branch_code){
                $scope.tableMessage1 = "请录入本网点理财!";
                $('#ckyxlr_new_add_button').hide();
            }
            else{    
            $scope.tableMessage1 = "";
            SqsReportService.info('acctcheck',{'account':account,'typ':"理财",'org_no':$rootScope.user_session.branch_code}).success(function(resp) {
               $scope.newstaffdata = resp;
                    if ((resp.rows || []).length > 0) {
                        $scope.tableMessage1 = "该流水号的当前录入业绩期间在系统中已经存在挂钩关系或占比已达到100，请检查！"+"员工"+resp.rows[0][1]+"拥有该业绩";
                        $('#lcyxlr_new_add_button').hide();
                    } else {
                        $('#Lcyxlr_input').show();
                        ...
                        $('#lcyxlr_new_add_button').show();
                    }
                });
            }
            } else {
                $scope.tableMessage1 = "未查询到此理财流水号";
                $('#lcyxlr_new_add_button').hide();
            }
        });
        }
    ...
    $scope.save = function(){
        ...
        accthkService.check_manager({'staff':staff,'org_no':$rootScope.user_session.branch_code,'account_no':$scope.add_zh,'typ':'理财'}).success(function (reps){
            if(reps.data=='0'){
                alert('请输入本网点员工!')
                return;
                }
                ...
                accthkService.ssave({"newdata":nnsd}).success(function (reps){
                    if(reps.data==1){
                        alert('同一流水号占比超过100,请检查!')
                        return;
                    }
                    $('#lcyxlr_modal').modal('hide');
                    alert('录入成功！');
                    $scope.search(); 
                });
            }
    ...
    $scope.edit_save = function(){
        ...
        accthkService.check_manager({'staff':[$scope.newstaffdata[0]],'org_no':$rootScope.user_session.branch_code,'account_no':$scope.add_zh,'typ':'理财'}).success(function (reps){
            if(reps.data=='0'){
                alert('请输入本网点员工!')
                return;
                }
...
lcyxlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','accthkService','dictdataService','custhkService'];

angular.module('YSP').service('lcyxlrController', lcyxlrController);
