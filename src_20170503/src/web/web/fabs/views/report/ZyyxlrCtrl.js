 ...
 * zyyxlr Controller
 */
function zyyxlrController($scope,store, $filter, $rootScope, SqsReportService,custhkService,dictdataService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/custhk1?export=1&TYP="+encodeURI("中间业务");
    ...
    $scope.newdata={};
    $('#check_zyyxlr').show();
    $('#zyyxlr_modal').modal('show');
    $('#zyyxlr_new_add_button').hide();
    $('#zyyxlr_save_edit_button').hide();
    $scope.tableMessage1 = "请输入客户号";
    };
    $scope.check = function(account){
        ...
        if (($scope.newdata.rows || []).length > 0) {
                $scope.tableMessage1 = "";
            SqsReportService.info('custhk1',{'CUST_NO':account,'TYP':"中间业务"}).success(function(resp) {
               $scope.newstaffdata = resp;
                    if ((resp.rows || []).length > 0) {
                        ...
                        $scope.newstaffdata[3] = moment(resp.rows[0][5].toString());
...
zyyxlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','custhkService','dictdataService'];

angular.module('YSP').service('zyyxlrController', zyyxlrController);
