 ...
 */
function stockyxlrController($scope,store, $filter, $rootScope, SqsReportService,custhkService,custhkService,branchmanageService,custhkService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/accthk1?export=1&TYP="+encodeURI("第三方存管");
    ...
    $scope.check = function(account){
        ...
        SqsReportService.info('custhk2',{'CUST_NO':account,'TYP':"第三方存管"}).success(function(resp){
        $scope.newdata=resp;
        console.log(resp)
        if (($scope.newdata.rows || []).length > 0) {
                $scope.tableMessage1 = "";
            SqsReportService.info('custhk1',{'CUST_NO':account,'TYP':"第三方存管",'ORG_NO':$rootScope.user_session.branch_code}).success(function(resp) {
...
stockyxlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','custhkService','custhkService','branchmanageService','custhkService'];
