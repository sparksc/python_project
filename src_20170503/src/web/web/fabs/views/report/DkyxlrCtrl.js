 ...
 * ckyxlr Controller
 */
function dkyxlrController($scope,store, $filter, $rootScope, SqsReportService,accthkService,dictdataService,custhkService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/dkaccthk1?export=1&TYP="+encodeURI("贷款");
    $scope.newstaffdata= {};
    ...
    $scope.add = function(){
        ...
        $('#check_dkyxlr').show();
        $('#chk_btn_dkyxlr').show()
        $('#dkyxlr_modal').modal('show');
        $('#Dkyxlr_input').hide();
        $('#Dkyxlr_edit').hide();
        $('#dkyxlr_new_add_button').hide();
        $('#dkyxlr_save_edit_button').hide();
    ...
    $scope.check = function(account,flag){
        ...
        SqsReportService.info('dkaccthk3',search_no).success(function(resp){
        $scope.newdata=resp;
        ...
        console.log($rootScope.user_session.branch_code)
            ...
            if(resp.rows[0][4]!=$rootScope.user_session.branch_code){
                ...
                $scope.tableMessage1 = "";
                SqsReportService.info('dkacctcheck',{'account':account,'typ':"贷款",'org_no':$rootScope.user_session.branch_code}).success(function(resp) {
                $scope.newstaffdata = resp;
                    if ((resp.rows || []).length > 0) {
                        $scope.tableMessage1 = "该账号的当前录入业绩期间在系统中已经存在挂钩关系或占比已达到100，请检查！";
                        $('#dkyxlr_new_add_button').hide();
                    } else {
                        $('#Dkyxlr_input').show();
                        ...
                        $('#dkyxlr_new_add_button').show();
                    }
                });
            }
            } else {
                $scope.tableMessage1 = "未查询到此贷款账号";
                $('#dkyxlr_new_add_button').hide();
            }
        });
        }
    ...
    $scope.save = function(){
        ...
        accthkService.check_manager({'staff':staff,'org_no':$rootScope.user_session.branch_code,'account_no':$scope.newdata.rows[0][0],'typ':'贷款'}).success(function (reps){
            if(reps.data=='0'){
                alert('请输入本网点员工!')
                return;
                }
    ...
    $scope.to_edit = function(row){
        SqsReportService.info('dkaccthk3',{'ACCOUNT_NO':row[5]}).success(function(resp){
            $scope.newdata=resp;
            $scope.tableMessage1 = "";
        }
    );
...
dkyxlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','accthkService','dictdataService','custhkService'];

angular.module('YSP').service('dkyxlrController', dkyxlrController);
