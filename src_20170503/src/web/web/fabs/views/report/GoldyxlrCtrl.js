 ...
 * goldyxlr Controller
 */
function goldyxlrController($scope,store, $filter, $rootScope, SqsReportService,custhkService,accthkService,branchmanageService,custhkService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/accthk1?export=1&TYP="+encodeURI("贵金属");
    ...
    $scope.add_zh = '';
    $('#check_goldyxlr').show();
    $('#goldyxlr_modal').modal('show');
    $('#goldyxlr_new_add_button').hide();
    $('#goldyxlr_save_edit_button').hide();
    $scope.tableMessage1 = "请输入客户号";
    };
    $scope.check = function(account){
        ...
        SqsReportService.info('custhk2',{'CUST_NO':account,'TYP':"贵金属"}).success(function(resp){
        $scope.newdata=resp;
        console.log(resp)
        if (($scope.newdata.rows || []).length > 0) {
                $scope.tableMessage1 = "";
            SqsReportService.info('custhk1',{'CUST_NO':account,'TYP':"贵金属",'ORG_NO':$rootScope.user_session.branch_code}).success(function(resp) {
               $scope.newstaffdata = resp;
                    if ((resp.rows || []).length > 0) {
                        ...
                        $scope.tableMessage1 = "该客户号的当前录入业绩期间在系统中已经存在营销挂钩关系，请检查！";
                        $('#goldyxlr_new_add_button').hide();
                    } else {
    ...
    $('#goldyxlr_modal').modal('show');
    $('#goldyxlr_new_add_button').hide();
    $('#goldyxlr_save_edit_button').show();
...
goldyxlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','custhkService','accthkService','branchmanageService','custhkService'];

angular.module('YSP').service('goldyxlrController', goldyxlrController);
