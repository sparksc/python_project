 ...
 * atmyxlr Controller
 */
function atmyxlrController($scope,store, $filter, $rootScope, SqsReportService,custhkService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.cust_search = {};
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/custhk1?export=1&TYP="+encodeURI("ATM");
    ...
    $scope.newdata={};
    $('#check_atmyxlr').show();
    $('#atmyxlr_modal').modal('show');
    $('#atmyxlr_new_add_button').hide();
    $('#atmyxlr_save_edit_button').hide();
    $scope.tableMessage1 = "请输入客户号";
    };
    $scope.check = function(account){
        ...
        if (($scope.newdata.rows || []).length > 0) {
                $scope.tableMessage1 = "";
            SqsReportService.info('custhk1',{'CUST_NO':account,'TYP':"ATM"}).success(function(resp) {
...
atmyxlrController.$inject = ['$scope','store', '$filter', '$rootScope', 'SqsReportService','custhkService','branchmanageService'];

angular.module('YSP').service('atmyxlrController', atmyxlrController);
