/**
 * Zhgyajjxxc Controller
 */
function ZhgyajjxxcController($scope, $filter, SqsReportService,store,branchmanageService,depappointService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
...
/*
    $scope.search = function() {
        ...
        SqsReportService.info('000008', params).success(function(resp) {
            $scope.data = resp;
...
*/
    ...
    function do_search(){
        ...
        SqsReportService.info('teller_y_xx_check', params).success(function(resp) {
            $scope.data = resp;
            if (($scope.data.rows || []).length > 0) {
                $scope.tableMessage = "";
            } else {
...
ZhgyajjxxcController.$inject = ['$scope', '$filter', 'SqsReportService','store','branchmanageService','depappointService'];



angular.module('YSP').service('ZhgyajjxxcController', ZhgyajjxxcController);
