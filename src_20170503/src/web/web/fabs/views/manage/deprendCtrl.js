 ...
 *  deprend  Controller
 */
function deprendController($scope, $rootScope, $attrs, $filter, SqsReportService, branchmanageService, gsgxckService) {
    ...
    $scope.cust_search.login_no = $rootScope.user_session.user_code;
    $scope.tableMessage = "请点击查询";
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/deprend?export=1&login_branch_no="+$rootScope.user_session.branch_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/deprend?export=1";
        ...
        SqsReportService.info('deprend', params).success(function(resp) {
    ...
    $scope.detail = function(id){
        params ={'CUST_IN_NO':$scope.data.rows[id][10],'ORG':$scope.data.rows[id][1]}
    ...
    $scope.show = function (item){
        if($scope.current_user){
            var r = confirm("确定显示？");
            if(r == true){
                branchmanageService.show({'cust_id':item[0]}).success(function(reps){
                    alert(reps.data);
                    $scope.search();
                });
            }else{
    ...
    $scope.hide = function (item) {
        if($scope.current_user){
            var r = confirm("确定隐藏？");
            if(r == true){
                branchmanageService.hide({'cust_id':item[0]}).success(function(reps){
                    alert(reps.data);
                    $scope.search();
                })
            }else{
    ...
    $scope.allot = function(item) {
        if($scope.current_user){
            $scope.add_manager = '';
            $scope.cust_id = item[0];
            branchmanageService.show_org({'cust_id':item[0]}).success(function(reps){
    ...
    $scope.find_users2 = function(){
        branchmanageSercice.users({'branch_id':$scope.top_id}).success(function(reps){
            $scope.model2 = reps.data;
        });
    };
    ...
    $scope.ztreeBtmConfirm = function(){
        ...
        $("#"+$scope.subTabId).find("div[name='loan_type_modal']").modal("hide");
...
deprendController.$inject = ['$scope', '$rootScope', '$attrs', '$filter', 'SqsReportService', 'branchmanageService', 'gsgxckService'];

angular.module('YSP').service('deprendController', deprendController);
