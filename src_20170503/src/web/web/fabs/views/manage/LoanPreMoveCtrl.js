 ...
 */
function LoanPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,chooseService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/LoanPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'贷款转移','<div ng-include="\'views/manage/LoanPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('LoanMove', params).success(function(resp) {
   ...
   //全选与单选功能，全选为当前页面,prkey声明主键
    ...
    $scope.selectall = function(master,data){
        if(master){
            ...
            for(rowvalue in data){
                var hasin = $scope.choseArr.indexOf(data[rowvalue][prkey]);
                if(hasin==-1){
                    $scope.choseArr.push(data[rowvalue][prkey]);
                    $scope.choseArr2.push(data[rowvalue][amtkey]);
                }
            }
        }
        else{
    ...
    $scope.chkone = function(row,oneflag){
        ...
        if(oneflag){
            ...
            $scope.choseArr2.push(row[amtkey])
        }
        if(!oneflag){
            $scope.choseArr.splice(hasin,1)
            $scope.choseArr2.splice(hasin,1)
        }
    };
    **/

    var move_element =angular.element(document.getElementById($scope.subTabId)).find('#move_loan');
    //批量移交功能
    $scope.save_flag=''
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top_cust({'id':$scope.choseArr[0]}).success(function (reps) {
            ...
            $scope.top_org = reps.data[0].org_no;    
                gsgxckService.batch_account_move_sum_with_hook({'update_key':$scope.choseArr,'query_date':'20160630','typ':'贷款','follow_cust':'客户号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        ...
        if($scope.is_all==""||$scope.is_all==null){
            alert("请选择是否移交其他业务")
            return
        }
        ...
        $scope.newdata.amount = $scope.total_amount;
        ...
        if($scope.staff==$scope.add_manager.user_name){
            alert("接受柜员和转出柜员一样");
            return;
        }
        $("div[name='loading']").modal("show");
        if($scope.is_all=="1"){
            gsgxckService.batch_cust_move_all({'typ':'贷款','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
                alert(resp.data);
                move_element.modal('hide');
	            $scope.search();
                $scope.checkedAllFlag = false;
                $scope.choseArr =[];
            }).error(function(resp){$("div[name='loading']").modal("hide");
            });
        }
        else{
            gsgxckService.batch_cust_move({'typ':'贷款','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
                alert(resp.data);
                move_element.modal('hide');
                $scope.checkedAllFlag = false;
                $scope.choseArr =[];
    ...
    $scope.find_users_by_branches = function(){
        branchmanageService.find_users_by_branches({'branch_id':$scope.cust_search.ORG_NO, 'is_khjl':1}).success(function(reps){
            $scope.model2 =reps.data;
       });
    };
    ...
    $scope.find_users_by_branches1 = function(){
        branchmanageService.find_users_by_branches({'branch_id':$scope.cust_search.org_tar, 'is_khjl':1}).success(function(reps){
            $scope.model5 =reps.data;
       });
    };
    ...
    $scope.choose_submit_loan= function(choose_data) {
        ...
        if (confirm("确定要根据选择条件预移交吗?----移交的条数:"+$scope.total_count.toString()
                +'\n'+'筛选信息:'+'\n'+ '客户号:'+($scope.cust_search.cust_no ||"")+'\n'+'公私类型'+($scope.cust_search.CST_TYP||"")+'\n'+'地址信息:'+($scope.cust_search.note||"") +'\n'+'地址信息剔除:'+($scope.cust_search.notnote ||"")
                 ...
                 chooseService.choose_loanpremove(nsd).success(function(resp) {
                    alert(resp.data)
                    $scope.choseArr =[];
                    $scope.search()
                 }).error(function(resp){
    ...
    $scope.tijiao_choose_submit_loan = function(choose_data) {
        if($scope.total_count==0)
        {
         alert('当前条数为0条,不能提交')
         return
        ...
        if ($rootScope.user_session.branch_code.charAt(0) != 'M'){
         ...
         if (confirm("确定要根据选择条件提交吗?----移交的条数:"+$scope.total_count.toString()
                +'\n'+'筛选信息:'+'\n'+ '客户号:'+($scope.cust_search.cust_no ||"")+'\n'+'公私类型'+($scope.cust_search.CST_TYP||"")+'\n'+'地址信息:'+($scope.cust_search.note||"") +'\n'+'地址信息剔除:'+($scope.cust_search.notnote ||"")
                 ...
                 $scope.cur_page = 1;
                 console.log(params)
                 nsd={'params':params,'flag_status':choose_data ,'condition':{'query_date':'20160630','typ':'贷款','follow_cust':'客户号优先'}}
                chooseService.choose_loanpremove(nsd).success(function(resp) {
    ...
    $scope.do_batch_move_loan= function(choose_data){
        if($scope.add_manager==""||$scope.add_manager==null){
            alert("请选择移交柜员");
            return;
        }
        ...
        $scope.cur_page = 1;
        console.log(params)
        nsd={'params':params,'flag_status':choose_data ,'loan_move_all':$scope.is_all,'condition':{'typ':'贷款','note':$scope.note,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}}
        chooseService.choose_loanpremove(nsd).success(function(resp) {
            alert(resp.data);
            move_element.modal('hide');
            $scope.choseArr =[];
    ...
    $scope.cexiao_choose_submit_loan = function(choose_data){
        if($scope.total_count==0)
        {
         alert('当前条数为0条,不能撤销')
         return
...
LoanPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','chooseService','$rootScope'];

angular.module('YSP').service('LoanPreMoveController', LoanPreMoveController);
