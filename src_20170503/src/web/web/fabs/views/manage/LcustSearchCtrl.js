 ...
 *  LcSearch  Controller
 */
function LcustSearchController($scope, $rootScope, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/lcust?export=1&ORG_NO="+$rootScope.user_session.branch_code+"&MANAGER_NO"+$rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/lcust?export=1";
        ...
        SqsReportService.info('lcust', params).success(function(resp) {
    ...
    $scope.detail = function(id){
        params ={'TYP':'理财','ID':$scope.data.rows[id][10],'ORG':$scope.data.rows[id][0],'MANAGER':$scope.data.rows[id][2]}
        SqsReportService.info('cust_detail', params).success(function(resp) {
            $scope.detail_data = resp;
            $('#tab_'+ $scope.tabId + '_content').find("#lcust_detail_modal").modal("show");
        });
    };
...
LcustSearchController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('LcustSearchController', LcustSearchController);
