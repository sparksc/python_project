/**
 *  ebankrend  Controller
 */
function ebankrendController($scope, $rootScope, $attrs, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebankrend?export=1&login_branch_no="+$rootScope.user_session.branch_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebankrend?export=1";
        ...
        SqsReportService.info('ebankrend', params).success(function(resp) {
    ...
    $scope.detail = function(id){
        params ={'CUST_IN_NO':$scope.data.rows[id][5],'ORG':$scope.data.rows[id][0]}
        SqsReportService.info('ebankInfo', params).success(function(resp) {
            ...
            $('#'+ $scope.subTabId).find("#edetail_modal").modal("show");
        });
    };
    ...
    $scope.show = function (item){
        if($scope.current_user){
            var r = confirm("确定显示？");
            if(r == true){
                branchmanageService.show({'cust_id':item[6]}).success(function(reps){
                    alert(reps.data);
                    $scope.search();
                })
            }else{
    ...
    $scope.hide = function (item) {
        if($scope.current_user){
            var r = confirm("确定隐藏？");
            if(r == true){
                branchmanageService.hide({'cust_id':item[6]}).success(function(reps){
                    alert(reps.data);
                    $scope.search();
                })
            }else{
    ...
    $scope.do_allot = function () {
        ...
        branchmanageService.do_allot({'type':'贷款','cust_id':$scope.cust_id,'to_teller_no':$scope.add_manager.user_name}).success(function(reps){
            alert(reps.data);
    ...
    $scope.allot = function(item) {
        if($scope.current_user){
            $scope.add_manager = '';
            $scope.cust_id = item[6];
            branchmanageService.show_org({'cust_id':item[6]}).success(function(reps){
    ...
    $scope.init_branches = function(fn){
        var tree_html = '<ul id="loan_type_tree'+$scope.subTabId+'" class="ztree"> </ul>';
        var fd = angular.element($("#"+$scope.subTabId).find("div[name='for_lt_tree']"))
        //console.log("fd",fd);
        fd.append(tree_html);
        var setting = {
            check:{
...
ebankrendController.$inject = ['$scope', '$rootScope', '$attrs', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('ebankrendController', ebankrendController);
