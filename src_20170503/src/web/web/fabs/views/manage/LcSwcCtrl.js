 ...
 * LcSwc Controller
 */
function LcSwcController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope,accthkService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        SqsReportService.info('lcswi', params).success(function(resp) {
...
LcSwcController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope','accthkService'];

angular.module('YSP').service('LcSwcController', LcSwcController);
