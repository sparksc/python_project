/**
 * InsPreMove Controller
 */
function InsPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/InsPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'保险代理转移','<div ng-include="\'views/manage/InsPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
   ...
   // $scope.cust_search.e_p_OPEN_DATE = moment();
    ...
    $scope.do_batch_move = function(){
        ...
        $("div[name='loading']").modal("show");
        gsgxckService.batch_cust_move({'typ':'代理保险','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
	        $scope.search();
            alert(resp.data);
            //$("div[name='loading']").modal("hide");
            $scope.checkedAllFlag = false;
...
InsPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('InsPreMoveController', InsPreMoveController);
