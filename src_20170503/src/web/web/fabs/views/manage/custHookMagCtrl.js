 ...
 */
function custHookMagController($scope, $filter, SqsReportService,branchmanageService,custHookMagService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.save = function(){
        mday = moment()
        custHookMagService.cust_move({'move_date':mday.format('YYYY-MM-DD'),'move_org_no':$scope.search_org,'move_cust_no':$scope.add_cust_no,'move_manager_no':$scope.add_manager.user_name,'move_percentage':$scope.add_percentage,'move_start_date':$scope.add_start_date.format('YYYY-MM-DD'),'move_end_date':$scope.add_end_date.format('YYYY-MM-DD'),'move_id': $scope.add_id}).success(function(resp){
            alert(resp.data);
            element.modal('hide');
	    $scope.search();
        });
...
custHookMagController.$inject = ['$scope', '$filter', 'SqsReportService','branchmanageService','custHookMagService'];

angular.module('YSP').service('custHookMagController', custHookMagController);
