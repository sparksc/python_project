 ...
 *  lcrend  Controller
 */
function lcrendController($scope, $rootScope, $attrs, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/lcrend?export=1&login_branch_no="+$rootScope.user_session.branch_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/lcrend?export=1";
        ...
        SqsReportService.info('lcrend', params).success(function(resp) {
    ...
    $scope.detail = function(id){
        params ={'CUST_IN_NO':$scope.data.rows[id][9],'ORG':$scope.data.rows[id][1]}
        SqsReportService.info('lcrendInfo', params).success(function(resp) {
...
lcrendController.$inject = ['$scope', '$rootScope', '$attrs', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('lcrendController', lcrendController);
