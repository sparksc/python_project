/*
 * DepaPreMove Controller
 */
function DepaPreMoveController($scope,$rootScope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/DepaPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'存款账号转移','<div ng-include="\'views/manage/DepaPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
    ...
    $scope.search = function() {
        ...
        $scope.cur_page = 1;
        console.log(params)
        SqsReportService.info('DepAcctMove', params).success(function(resp) {
    ...
    $scope.find_users = function(){
        branchmanageService.get_staff({'branch_code':$rootScope.user_session.branch_code}).success(function(reps){
    ...
    var move_element =angular.element(document.getElementById($scope.subTabId)).find('#move_DepaPreMove1');
    //批量移交功能
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top({'id':$scope.choseArr[0]}).success(function (reps) {
            $scope.top_org = reps.data[0].org_no;    
                gsgxckService.batch_account_move_sum_with_hook({'update_key':$scope.choseArr,'query_date':'20160630','typ':'存款','follow_cust':'账号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        ...
        $("div[name='loading']").modal("show");
        gsgxckService.batch_account_move({'typ':'存款','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'amount':$scope.total_amount,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
            alert(resp.data);
            //$("div[name='loading']").modal("hide");
    ...
    var Depa_element = angular.element('#Depa_choseModal');
...
DepaPreMoveController.$inject = ['$scope','$rootScope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService'];

angular.module('YSP').service('DepaPreMoveController', DepaPreMoveController);
