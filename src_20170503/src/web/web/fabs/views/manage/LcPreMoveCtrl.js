 ...
 */
function LcPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/LcPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'理财流水号转移','<div ng-include="\'views/manage/LcPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
   ...
   // $scope.cust_search.e_p_OPEN_DATE = moment();
    ...
    $scope.search = function() {
        ...
        $scope.cur_page = 1;
        console.log(params)
        SqsReportService.info('LcAcctMove', params).success(function(resp) {
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top({'id':$scope.choseArr[0]}).success(function (reps) {
            ...
            $scope.top_org = reps.data[0].org_no;    
                gsgxckService.batch_account_move_sum_with_hook({'update_key':$scope.choseArr,'query_date':'20160630','typ':'理财','follow_cust':'账号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        if($scope.cust_search.org_tar == null || $scope.cust_search.org_tar == '' ||  $scope.add_manager.user_name == null || $scope.add_manager.user_name == '')
        {
            alert('请先选择机构和柜员');
            return;
        }
        ...
        gsgxckService.batch_account_move({'typ':'理财','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
	        $scope.search();
            alert(resp.data);
            $scope.choseArr =[];
            $scope.checkedAllFlag = false;
...
LcPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('LcPreMoveController', LcPreMoveController);
