/**
 * custhooksp Controller
 * 客户挂钩审批
 */
function custhkspController($scope, $filter, SqsReportService,branchmanageService,custhkService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('custhksp', params).success(function(resp) {
            $scope.data = resp;
   ...
   //全选与单选功能，全选为当前页面,prkey声明主键
    ...
    $scope.approve = function() {
        ...
        custhkService.approve({'newdata':$scope.newdata,'update_key':$scope.choseArr}).success(function(resp){
            alert(resp.data);
        $scope.search();
        });
    }
    $scope.deny = function() {
        ...
        console.log('hello')
        console.log($scope.choseArr)
        custhkService.deny({'newdata':$scope.newdata,'update_key':$scope.choseArr}).success(function(resp){
            alert(resp.data);
        $scope.search();
        });
    }
...
custhkspController.$inject = ['$scope', '$filter', 'SqsReportService','branchmanageService','custhkService'];
angular.module('YSP').service('custhkspController', custhkspController);
