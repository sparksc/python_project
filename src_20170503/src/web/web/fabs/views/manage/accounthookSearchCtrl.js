 ...
 *  Dkgggx  Controller
 */
function accounthookSearchController($scope, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('accthk1', params).success(function(resp) {
            $scope.data = resp;
...
accounthookSearchController.$inject = ['$scope', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('accounthookSearchController', accounthookSearchController);
