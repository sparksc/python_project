/**
 * ThirdPreMove Controller
 */
function ThirdPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/ThirdPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'第三方存管转移','<div ng-include="\'views/manage/ThirdPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
    ...
    $scope.search = function() {
        ...
        console.log(params)
        SqsReportService.info('thirdPreMove', params).success(function(resp) {
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top_cust({'id':$scope.choseArr[0]}).success(function (reps) {
            ...
            branchmanageService.branch({'org':$scope.top_org}).success(function (reps) {
                ...
                gsgxckService.batch_account_move_sum_with_hook({'update_key':$scope.choseArr,'typ':'第三方存管', 'follow_cust':'客户号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        ...
        $("div[name='loading']").modal("show");
        gsgxckService.batch_cust_move({'typ':'第三方存管','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
	        $scope.search();
            //$("div[name='loading']").modal("hide");
            alert(resp.data);
            $scope.checkedAllFlag = false;
...
ThirdPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('ThirdPreMoveController', ThirdPreMoveController);
