/**
 * MachPreMove Controller
 */
function MachPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/MachPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'机具转移','<div ng-include="\'views/manage/MachPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
    ...
    $scope.do_batch_move = function(){
        ...
        $("div[name='loading']").modal("show");
        gsgxckService.batch_cust_move({'typ':'机具','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
            alert(resp.data);
            //$("div[name='loading']").modal("hide");
            $scope.checkedAllFlag = false;
            $scope.cust_search.org_tar = null;
            $scope.note = '';
            $scope.choseArr =[];
...
MachPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('MachPreMoveController', MachPreMoveController);
