 ...
 * DepSing Controller
 */
function DepSingleController($scope, $rootScope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope,custHookMagService) {
   $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        SqsReportService.info('depSingle', params).success(function(resp) {
            $scope.data = resp;
            $scope.data.header.push("操作");
    ...
    var element = angular.element('#custHookMoveModal');
    $scope.change = function(row) {
            $scope.row=row;
            $scope.move_id = row[10];
            $scope.move_percentage = row[7];
            element.modal('show');
            branchmanageService.branch({'org':$scope.row[1]}).success(function (reps) {
...
DepSingleController.$inject = ['$scope', '$rootScope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope','custHookMagService'];

angular.module('YSP').service('DepSingleController', DepSingleController);
