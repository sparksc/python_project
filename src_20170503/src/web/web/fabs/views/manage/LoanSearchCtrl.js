/**
 *  LoanSearch  Controller
 */
function LoanSearchController($scope, $rootScope, $filter, SqsReportService,branchmanageService,staff_statusService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/loan?export=1&login_branch_no="+$rootScope.user_session.branch_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/loan?export=1";
        ...
        SqsReportService.info('loan', params).success(function(resp) {
...
LoanSearchController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','branchmanageService','staff_statusService'];

angular.module('YSP').service('LoanSearchController', LoanSearchController);
