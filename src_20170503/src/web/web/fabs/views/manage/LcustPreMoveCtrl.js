/**
 * LcPreMove Controller
 */
function LcustPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/LcustPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'理财客户号转移','<div ng-include="\'views/manage/LcustPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
    ...
    $scope.search = function() {
        ...
        $scope.cur_page = 1;
        console.log(params)
        SqsReportService.info('LcustMove', params).success(function(resp) {
    ...
    $scope.detail = function(id){
        params ={'SALE_CODE':$scope.data.rows[id][3],
                'ORG':$scope.data.rows[id][1],
                'CUST_NO':$scope.data.rows[id][5]}
        console.log(params)
        SqsReportService.info('lcInfo', params).success(function(resp) {
            $scope.detail_data = resp;
            console.log("$scope.subTabId",$scope.subTabId);
            $('#'+ $scope.subTabId).find("#lc_detail_modal").modal("show");
        });
    };
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top_cust({'id':$scope.choseArr[0]}).success(function (reps) {
            ...
            $scope.top_org = reps.data[0].org_no;    
                gsgxckService.batch_account_move_sum_with_hook({'update_key':$scope.choseArr,'query_date':'20160630','typ':'理财','follow_cust':'客户号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        ...
        $("div[name='loading']").modal("show");
        gsgxckService.batch_cust_move({'typ':'理财','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
...
LcustPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('LcustPreMoveController', LcustPreMoveController);
