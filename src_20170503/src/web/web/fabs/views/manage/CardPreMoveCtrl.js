/**
 * CardPreMove Controller
 */
function CardPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/CardPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'信用卡转移','<div ng-include="\'views/manage/CardPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
   ...
   // $scope.cust_search.e_p_OPEN_DATE = moment();
    ...
    $scope.search = function() {
        ...
        $scope.cur_page = 1;
        console.log(params)
        SqsReportService.info('000052_tran', params).success(function(resp) {
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top({'id':$scope.choseArr[0]}).success(function (reps) {
            ...
            $scope.top_org = reps.data[0].org_no;    
                ...
                gsgxckService.batch_account_move_sum_with_hook({'update_key':$scope.choseArr,'follow_cust':'账号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        ...
        gsgxckService.batch_account_move({'typ':'信用卡','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
            alert(resp.data);
            //$("div[name='loading']").modal("hide");
...
CardPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('CardPreMoveController', CardPreMoveController);
