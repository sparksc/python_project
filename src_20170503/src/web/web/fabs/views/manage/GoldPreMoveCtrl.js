/**
 * GoldPreMove Controller
 */
function GoldPreMoveController($scope, $attrs, $filter, SqsReportService,branchmanageService,gsgxckService,$rootScope) {

    $scope.init = function (){
        $rootScope.addSubTab(0,'提交','<div ng-include="\'views/manage/GoldPreMove.html\'"> </div>',{},false,{'status':'预提交审批'}); 
        $rootScope.addSubTab(1,'贵金属转移','<div ng-include="\'views/manage/GoldPreMove.html\'"> </div>',{},false,{'status':'正常'}); 
    ...
    $scope.batch_move = function() {
        ...
        gsgxckService.get_top({'id':$scope.choseArr[0]}).success(function (reps) {
            ...
            branchmanageService.branch({'org':$scope.top_org}).success(function (reps) {
                ...
                gsgxckService.batch_cust_move_sum_with_hook({'update_key':$scope.choseArr,'follow_cust':'客户号优先'}).success(function (reps) {
    ...
    $scope.do_batch_move = function(){
        ...
        gsgxckService.batch_cust_move({'typ':'贵金属','note':$scope.note,'update_key':$scope.choseArr,'from_teller_no':$scope.staff,'to_teller_no':$scope.add_manager.user_name}).success(function(resp){
	        $scope.search();
            alert(resp.data);
            $scope.checkedAllFlag = false;
            $scope.choseArr =[];
    ...
    $scope.isManager = function() {
        ...
        if (branch_no.charAt(0) == 'M'||branch_no == '931000')
            $scope.flag=true;
    }
    $scope.isManager();
...
GoldPreMoveController.$inject = ['$scope', '$attrs', '$filter', 'SqsReportService','branchmanageService','gsgxckService','$rootScope'];

angular.module('YSP').service('GoldPreMoveController', GoldPreMoveController);
