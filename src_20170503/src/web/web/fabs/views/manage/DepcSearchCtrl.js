/**
 *  DepcSearch  Controller
 */
function DepcSearchController($scope, $rootScope, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/depcust?export=1&ORG_NO="+$rootScope.user_session.branch_code+"&MANAGER_NO"+$rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/depcust?export=1";
    ...
    $scope.detail = function(id){
        params ={'TYP':'存款','ID':$scope.data.rows[id][11],'ORG':$scope.data.rows[id][0],'MANAGER':$scope.data.rows[id][2]}
        SqsReportService.info('cust_detail', params).success(function(resp) {
            $scope.detail_data = resp;
            $('#tab_'+ $scope.tabId + '_content').find("#depc_detail_modal").modal("show");
        });
    };
...
DepcSearchController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('DepcSearchController', DepcSearchController);
