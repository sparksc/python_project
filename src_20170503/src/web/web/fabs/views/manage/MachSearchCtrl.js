/**
 *  MachSearch  Controller
 */
function MachSearchController($scope, $rootScope, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/machine?export=1&login_branch_no="+$rootScope.user_session.branch_code+"&login_teller_no"+$rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/machine?export=1";
        ...
        SqsReportService.info('machine', params).success(function(resp) {
...
MachSearchController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('MachSearchController', MachSearchController);
