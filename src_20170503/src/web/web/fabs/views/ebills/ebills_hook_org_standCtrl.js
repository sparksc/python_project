 ...
 * Mandep Controller
 */
function ebills_hook_org_standController($scope, $rootScope,store, $filter, SqsReportService, permissionService,branchmanageService,ebills_managerService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.onAction = function(conversation_id, action) {
        SqsReportService.action(conversation_id, action).success(function(resp) {
            ...
            merge_table_show( "ebills_hook_org_stand", $scope.data.merge_cols);
        });
    };   
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebills_hook_org_stand?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code+"&login_teller_no=" + $rootScope.user_session.user_code
        ...
        SqsReportService.info('ebills_hook_org_stand',params).success(function(resp) {
    ...
    $scope.org_stand_update=function(){
    if (confirm("确定要生效此月"+$scope.cust_search.DATE_ID.format("YYYYMM")+"的数据吗"))
     {
       ebills_managerService.org_stand_update({'month':$scope.cust_search.DATE_ID.format("YYYYMMDD")}).success(function(reps){
           alert(reps.data);
          });
     }
    }
   $scope.org_stand_delete=function(){
    if (confirm("确定要删除此月"+$scope.cust_search.DATE_ID.format("YYYYMM")+"的数据吗"))
     {
       ebills_managerService.org_stand_delete({'month':$scope.cust_search.DATE_ID.format("YYYYMMDD")}).success(function(reps){
           alert(reps.data);
          });
     }
    }
    ...
    $scope.upload_excel = function(){
        ...
        $.ajax({
            type: "POST",
            url : base_url+"/ebills_manager/ebills_hook_upload/",
            data: form,
            processData: false,
            contentType: false,
...
ebills_hook_org_standController.$inject = ['$scope', '$rootScope', 'store','$filter', 'SqsReportService', 'permissionService','branchmanageService','ebills_managerService'];

angular.module('YSP').service('ebills_hook_org_standController', ebills_hook_org_standController); 
