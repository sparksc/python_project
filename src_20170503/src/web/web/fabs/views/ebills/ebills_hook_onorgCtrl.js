 ...
 * Mandep Controller
 */
function ebillsHookOnorgController($scope, $rootScope,store, $filter, SqsReportService, permissionService,branchmanageService,ebills_managerService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebills_hook_onorg?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code+"&login_teller_no=" + $rootScope.user_session.user_code
        ...
        SqsReportService.info('ebills_hook_onorg',params).success(function(resp) {
...
ebillsHookOnorgController.$inject = ['$scope', '$rootScope', 'store','$filter', 'SqsReportService', 'permissionService','branchmanageService','ebills_managerService'];

angular.module('YSP').service('ebillsHookOnorgController', ebillsHookOnorgController); 
