 ...
 * Mandep Controller
 */
function wandian_org_standController($scope, $rootScope,store, $filter, SqsReportService, permissionService,branchmanageService,ebills_managerService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.onAction = function(conversation_id, action) {
        SqsReportService.action(conversation_id, action).success(function(resp) {
            ...
            merge_table_show( "wandian_org_stand", $scope.data.merge_cols)
        });
    };   
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebills_hook_org_stand?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code+"&login_teller_no=" + $rootScope.user_session.user_code+"&org_flag=Y";
...
wandian_org_standController.$inject = ['$scope', '$rootScope', 'store','$filter', 'SqsReportService', 'permissionService','branchmanageService','ebills_managerService'];

angular.module('YSP').service('wandian_org_standController', wandian_org_standController); 
