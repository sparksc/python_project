/**
 * Newfinappoint Controller
 */
function NewfinappointController($scope, $filter, SqsReportService,store,branchmanageService,depappointService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    function do_search(){
        ...
        SqsReportService.info('newfinappoint', params).success(function(resp) {
            $scope.data = resp;
    ...
    var add_modal = angular.element('#add_newfinappoint');
    $scope.delay = 7;
    $scope.newdata = {};
    ...
    $scope.do_save = function(){
        ...
        depappointService.exist({'keydata':{'khmc':$scope.newdata.khmc,'yybljg':$scope.newdata.yybljg_dis.branch_code}}).success(function(reps){
             if(!reps.data){
                ...
                $scope.newdata.bz ='1';
                //$scope.newdata.yyyxrq =$scope.newdata.yyrq.add(7,'days').format('YYYYMMDD');
...
NewfinappointController.$inject = ['$scope', '$filter', 'SqsReportService','store','branchmanageService','depappointService'];

angular.module('YSP').service('NewfinappointController', NewfinappointController);
