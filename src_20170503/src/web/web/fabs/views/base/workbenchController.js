 ...
 * Mandep Controller
 */
function workbenchController($scope, $rootScope, $filter, SqsReportService, permissionService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    function search() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/workbench_approve?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code
        ...
        SqsReportService.info('workbench_approve',params).success(function(resp) {
            $("div[name='loading']").modal("hide");
            $scope.approve_data = resp;
	    if (($scope.approve_data.rows || []).length > 0) {
                $scope.tableMessage = "";
            } else {
        ...
        params1 = {}; 
        $scope.excelurl1 = rpt_base_url + "/report_proxy/sqs/workbench_hook?export=1"+"&DATE_ID="+ $scope.cust_search.DATE_ID.format("YYYYMMDD") + "&login_branch_no=" + $rootScope.user_session.branch_code + "&login_teller_no=" + $rootScope.user_session.user_code
        for(var key in $scope.cust_search){
            if($scope.cust_search[key]!=''&&$scope.cust_search[key]!=null){
            params1[key]=$scope.cust_search[key];
            if(key=='DATE_ID')params1['DATE_ID']=params1['DATE_ID'].format("YYYYMMDD");
            $scope.excelurl1 = $scope.excelurl1 + "&"+key+"="+params1[key]
            }
        }

        SqsReportService.info('workbench_hook',params1).success(function(resp1) {
            $scope.hook_data = resp1;
	    if (($scope.hook_data.rows || []).length > 0) {
                $scope.tableMessage = "";
            } else {
                $scope.tableMessage = "未查到数据";
            }
...
workbenchController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService', 'permissionService','branchmanageService'];

angular.module('YSP').service('workbenchController', workbenchController); 
