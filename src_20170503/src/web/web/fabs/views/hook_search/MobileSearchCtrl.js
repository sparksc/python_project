/**
 *  MobileSearch  Controller
 */
function MobileSearchController($scope, $rootScope, $filter, SqsReportService,branchmanageService) {
    $scope.dict = {
        "first": "首页",
        "previous": "上一页",
        "next": "下一页",
    ...
    $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebk_mobile?export=1&ORG_NO="+$rootScope.user_session.branch_code+"&MANAGER_NO"+$rootScope.user_session.user_code;
    ...
    $scope.search = function() {
        ...
        $scope.total_count = 0;
        $scope.cur_page = 1;
        $scope.excelurl = rpt_base_url + "/report_proxy/sqs/ebk_mobile?export=1";
        ...
        SqsReportService.info('ebk_mobile', params).success(function(resp) {
...
MobileSearchController.$inject = ['$scope', '$rootScope', '$filter', 'SqsReportService','branchmanageService'];

angular.module('YSP').service('MobileSearchController', MobileSearchController);
