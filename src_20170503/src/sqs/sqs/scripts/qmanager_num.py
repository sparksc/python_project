...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        if int(self.date_id) != int(row_date[0]):
            return (u"未到月末,不能查看")
        sql ="""
        select date_id,org_code,org_name,nvl(manager_num,0) ,manager_bak,max(ID) from report_hall_manager_hander WHERE 1=1 and manager_num is not null  %s  group by (date_id,org_code,org_name,manager_num ,manager_bak) order by org_code
        """%(filterstr)
        print sql
...
#            rowlist.append(t)
    ...
    def column_header(self):
        return ["统计日期","机构号","机构名称","大堂经理人数","备注","操作"]
