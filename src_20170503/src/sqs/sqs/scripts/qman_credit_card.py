...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        rr=[]
        while True:
            c = (int(row[i][4])-int(row1[i][4]))
            rr.append((ym,row[i][0],row[i][1],row[i][2],row[i][3],c,0,0,0))
            i+=1
            if i>=len(row1):
                break
        needtrans ={}
    ...
    def make_eq_filterstr(self):
        ...
        return filterstr3,filterstr1,filterstr2,filterstr,vlist1,vlist2,mnn
    def column_header(self):
        return ["统计月份","机构号","机构名","客户号","客户名","发卡量","不良率","新增贷记卡","新增丰收贷记卡逾期本金"]
    @property
    def page_size(self):
        return 15
