...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        sql =u"""SELECT c.ORG_NO, d.BRANCH_NAME, c.manager_no, s.name, c.CUST_NO, i.CUST_NAME,percentage,nvl(c.balance/100.00,0),nvl(c.add_avg_balance/100.00,0), note, c.ID 
                 ...
                 WHERE typ='理财' and status in ('正常','已审批','预提交审批','待审批') %s ORDER BY ID DESC"""%(filterstr)
        print sql
    ...
    def column_header(self):
        return ["机构号","机构名","员工号","员工名","客户号","客户名","占比(%)","总购买金额(元)","当前日均(元)","地址信息"]
