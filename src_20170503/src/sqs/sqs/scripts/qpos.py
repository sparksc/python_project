...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        self.filterlist = ['CUST_NO','MANAGER_NO','ORG_NO','STATUS','NOTE','TYPE']
        filterstr,vlist = self.make_eq_filterstr()
        sql =u"""SELECT c.ORG_NO, d.BRANCH_NAME, c.manager_no, s.name, c.CUST_NO,k.MERCHANT_NAME,k.TYP, NOTE, c.ID 
                 FROM YDW.CUST_HOOK c
                 JOIN BRANCH d on d.branch_code=c.org_no 
                 JOIN F_USER s on s.user_name=c.manager_no
                 JOIN D_POS k on c.cust_no=k.MERCHANT_NO AND c.CUST_IN_NO=k.POS_NO 
                 WHERE c.typ='POS' and c.status in ('正常','已审批','预提交审批','待审批') %s ORDER BY ID"""%(filterstr)
        print sql
