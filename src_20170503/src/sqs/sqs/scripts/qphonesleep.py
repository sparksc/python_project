...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        sql ="""
            SELECT f.DATE_ID,m.THIRD_BRANCH_CODE,m.THIRD_BRANCH_NAME,m.SALE_CODE,m.SALE_NAME,d.CST_NAME,d.NET_CST_NO,d.CST_NO,d.ID_NUMBER,d.OPEN_DATE,d.OPEN_BRANCH_NO,d.ACCT_NO,d.MOBILE_NO,f.STATUS,MAX(F.LAST_LOGON_DATE,LEFT(D.OPEN_DATE,8)) 
            FROM F_CONTRACT_STATUS F
            JOIN D_CUST_CONTRACT D ON F.CONTRACT_ID=D.ID
            JOIN D_SALE_MANAGE_RELA M ON M.MANAGE_ID=f.MANAGE_ID
            WHERE D.BUSI_TYPE='手机银行' AND F.STATUS IN ('正常','暂时冻结','停用','冻结') 
    ...
    def column_header(self):
        return ["统计日期","机构号","机构名","员工号","员工名","客户名","客户号","客户内码","身份证号","办理日期","办理机构","卡号","电话","状态","最后登录时间"]
