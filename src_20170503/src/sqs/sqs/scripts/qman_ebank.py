...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        sql5 =u"""
                ...
                (select open_branch_no,manage_code,count(1) amt
                from d_cust_contract c 
                where c.busi_type = '第三方存管' %s %s
                group by open_branch_no,manage_code) a
                right join teller t on t.user_name=a.manage_code
                where 1=1 %s
                order by branch_code,user_name
        ...
        while True:
            ...
            c9 = '0'#self.trans_dec()
            rr.append((self.ym,row[i][0],row[i][1],row[i][3],row[i][4],c1,c2,c3,c4,c5,c6,c7,c8,c9))
        needtrans ={}
        ...
        for k,v in self.args.items():
            if v and k in self.filterlist:
                ...
                if k == 'org':
                    ...
                    filterstr1 = filterstr1 + " and org_code in (%s) "%(vvv)
                if k == 'id':
                    #获取客户经理号
                    self.id=v
                    self.manage=v
                    ...
                    vlist1.append(v)
                    vlist2.append(v)
                    filterstr3 = filterstr3 + " and manage_code=? "
                    vlist5.append(v)
                    vlist6.append(v)
        if self.org<>"":
            self.org = " and branch_code in (%s) "%(vvv)
            self.branch = " and open_branch_no in (%s) "%(vvv)
        if self.manage<>"":
            self.org = " and user_name="+self.id+" "
            self.branch = " and manage_code="+self.manage+" "
        return filterstr3,filterstr2,filterstr1,vlist6,vlist5,vlist4,vlist3,vlist2,vlist1

    def column_header(self):
        return ["统计月份","机构号","机构名称","客户经理编号","客户经理名称","新增手机银行有效户数","新增企业网银有效户数","新拓展POS机1","新拓展POS机2","新增有效丰收e支付","第三方存管存量户数","第三方存管增量户数","新增ETC","助农服务点月平均活点率"]
