...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        sql =u"""SELECT c.ORG_NO, d.BRANCH_NAME, c.manager_no, s.name, c.ACCOUNT_NO, i.CUST_NAME,nvl(c.balance/100.00,0),nvl(c.add_avg_balance/100.00,0), NOTE, c.ID 
                 ...
                 WHERE typ='理财' and follow_cust='账号优先' and status in ('正常','已审批','预提交审批','待审批') %s ORDER BY ID"""%(filterstr)
        print sql
        ...
        for k,v in self.args.items():
            if v and k in self.filterlist:
                if k == 'ORG_NO':
                    ...
                    filterstr = filterstr + " and c.account_no like " + " '%'||"+"?"+"||'%' "
                    vlist.append(v)
                elif k == 'NOTE':
    ...
    def column_header(self):
        return ["机构号","机构名","员工号","员工名","流水号","客户名","余额(元)","当前日均(元)","地址信息"]
