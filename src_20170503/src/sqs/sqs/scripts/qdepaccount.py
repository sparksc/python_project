...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        self.filterlist = ['ACCOUNT_NO','MANAGER_NO','ORG_NO','STATUS','NOTE']
        filterstr,vlist = self.make_eq_filterstr()
        sql =u"""SELECT c.ORG_NO, d.BRANCH_NAME, c.manager_no, s.name, c.ACCOUNT_NO,i.CST_NAME, c.percentage,nvl(c.balance/100.00,0),nvl(c.add_avg_balance/100.00,0),c.note,follow_cust, c.ID 
                 FROM YDW.ACCOUNT_HOOK c
                 JOIN BRANCH d on d.branch_code=c.org_no 
                 JOIN F_USER s on s.user_name=c.manager_no
                 JOIN D_ACCOUNT i on i.account_no=c.account_no
                 WHERE typ='存款' and follow_cust='账号优先' and status in ('正常','已审批','预提交审批','待审批') and name is not null %s ORDER BY ID DESC"""%(filterstr)
        print sql
    ...
    def column_header(self):
        return ["机构号","机构名","员工号","员工名","账号","客户名","占比(%)","余额(元)","当前日均(元)","地址信息","优先级"]
