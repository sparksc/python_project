...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        if int(self.date_id) != int(row_date[0]):
            return (u"未到月末,不能查看")
        sql ="""select date_id,org_code,org_name,sale_code,sale_name,nvl(work_days,0),nvl(server_days,0),ID from report_hall_manager_hander WHERE 1=1 and (work_days is not null or server_days is not null )  and SALE_CODE is not null  %s order by org_code,sale_code"""%(filterstr)
        print sql
