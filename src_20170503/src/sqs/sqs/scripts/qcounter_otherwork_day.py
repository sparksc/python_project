...
class Query(ObjectQuery):
    ...
    def prepare_object(self):
        ...
        if int(self.date_id) != int(row_date[0]):
            return (u"未到月末,不能查看")
        sql ="""select date_id,org_code,org_name,sale_code,sale_name,nvl(work_days,0),nvl(server_days,0),ID from report_manager_workquality_hander WHERE 1=1 and (work_days is not null or server_days is not null )  and SALE_CODE is not null  %s order by org_code,sale_code"""%(filterstr)
        print sql
    ...
    def column_header(self):
        return ["统计日期","机构号","机构名称","员工号","员工名称","工作日","担任天数","操作"]
