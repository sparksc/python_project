...
def run_import(filename,org_no=None,subtype=None):
    ...
    del_sql="""
    DELETE FROM YDW.D_ATM WHERE typ in ('自助终端','助农终端','发卡机') 
    """
    ...
    for r in range(1,nrows):
        ...
        TERMINAL_NO = str(row[3].strip().encode('UTF-8'))
        TYP = str(row[6].strip().encode('UTF-8'))
        SUB_TYP = str(row[4].strip().encode('UTF-8'))
        ADDR = str(row[5].strip().encode('UTF-8'))
        TO_ORG_NO = str(row[7]).strip()

        if TO_ORG_NO=='':
            #print ORG_NO, row
            continue    

        rc.append((ORG_NO, TERMINAL_NO,TYP,SUB_TYP,ADDR,'正常'))
