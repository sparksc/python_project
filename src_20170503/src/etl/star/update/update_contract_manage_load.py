...
def get_ebank_cust_manage(db,sql,etldate):
    ...
    while row:
        m_id,flag = DimManage().find_contract_cust_key2(row[3],row[0])      #不考虑子类型
...
def run(etldate):
    ...
    try :
        ...
        where f.DATE_ID = ? and c.BUSI_TYPE in ('个人网上银行','企业网上银行','手机银行','支付宝卡通','丰收e支付','新丰收e支付','ETC','支付宝快捷支付') and substr(c.cst_no,1,1)='8'
        """ 
        ...
        join D_CUST_CONTRACT c on f.CONTRACT_ID=c.ID
        where f.DATE_ID = ? and c.BUSI_TYPE='第三方存管'
        """ 
        writer_manage_id(db,get_stock_cust_manage,stocksql,etldate)
