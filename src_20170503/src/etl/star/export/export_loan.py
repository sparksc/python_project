...
def run_export():
    ...
    for i in range(0,len(data)):
        ...
        if x[0]+x[3] not in rowlist:
            ...
            if x[-1]=='无':
                tx=manage_code+'!电子银行管户!100!'+str(x[3])+'!966110!966000'
                tlist=list(x[:-3])+typelist+tx.split('!')+[htype]
            else:    
                tlist=list(x[:-3])+typelist+x[-1].split('!')+[htype]
    ...
    #    idx=idx+1    
    #sumbook.save("966110电子银行经理合计.xls")
def query_detail():
    try :
        db = util.DBConnect()
        q_sql="""
        select c.CST_NO,i.CUST_LONG_NO,i.CUST_NAME,o.org0_code,i.CUST_ADDRESS,i.CUST_CREDIT_ADDRESS,c.BUSI_TYPE,c.NET_CST_NO,m.GROUP_KEY from F_CONTRACT_STATUS f
        ...
        join D_ORG o on o.id=f.org_id and o.org1_code='966110'
        where f.DATE_ID=20160630 
        order by c.CST_NO,c.OPEN_BRANCH_NO
        """
        db.cursor.execute(q_sql)
        row=db.cursor.fetchall()
        q1_sql="""
        select CST_NO,i.CUST_LONG_NO,i.CUST_NAME,OPEN_BRANCH_NO,i.CUST_ADDRESS,i.CUST_CREDIT_ADDRESS,BUSI_TYPE,NET_CST_NO,'无' from D_CUST_CONTRACT c 
        join D_CUST_INFO i on c.CST_NO=i.CUST_NO
        where BUSI_TYPE in ('支付宝卡通','支付宝快捷支付') and OPEN_BRANCH_NO in ('966110','966111','966112','966113','966115')
        """
...
def query_hook():
    try :
        db = util.DBConnect()
        q_sql="""
        select distinct CUST_IN_NO,ORG_NO,MANAGER_NO from CUST_HOOK where TYP='存款' and ORG_NO in ('966110','966111','966112','966113','966115') and HOOK_TYPE='管户'
        """
        db.cursor.execute(q_sql)
        row=db.cursor.fetchone()
        rowdict={}
        while row:
            ...
            row=db.cursor.fetchone()
        q_sql="""
        select distinct CUST_IN_NO,ORG_NO,MANAGER_NO from CUST_HOOK where TYP='贷款' and ORG_NO in ('966110','966111','966112','966113','966115') and HOOK_TYPE='管户'
        """
        db.cursor.execute(q_sql)
        row=db.cursor.fetchone()
        while row:
