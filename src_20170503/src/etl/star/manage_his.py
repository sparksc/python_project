...
class DimManage():
    ...
    def load_dep_relation(self):
        #存款客户号关系
        sql="select org_no||'!'||cust_in_no,manager_no,typ||hook_type type,percentage from cust_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '存款'"
        self.ckcust_managers=self.get_rela_dict(sql)

        #存款账号关系
        sql="select '!'||account_no,manager_no,typ||hook_type type,percentage from ACCOUNT_HOOK_HIS where end_date <=%d and STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '存款'"
        self.ckacct_managers=self.get_rela_dict(sql, int(Config().etldate))

    #贷款关系    
    def load_loan_relation(self):
        #贷款客户号关系
        sql="select org_no||'!'||cust_in_no,manager_no,typ||hook_type type,percentage from cust_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '贷款'"
        self.dkcust_managers=self.get_rela_dict(sql)

        #贷款账号关系
        sql="select org_no||'!'||account_no,manager_no,typ||hook_type type,percentage from ACCOUNT_HOOK_HIS where end_date <=%d and STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '贷款'"
        self.dkacct_managers=self.get_rela_dict(sql, int(Config().etldate))

    #理财关系    
    def load_fin_relation(self):
        #理财客户号关系
        sql="select org_no||'!'||cust_in_no,manager_no,typ||hook_type type,percentage from cust_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '理财'"
        self.fincust_managers=self.get_rela_dict(sql)

        #理财账号关系
        sql="select '!'||account_no,manager_no,typ||hook_type type,percentage from ACCOUNT_HOOK_HIS where end_date <=%d and STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '理财'"
        self.finacct_managers=self.get_rela_dict(sql, int(Config().etldate))
        #print len(self.finacct_managers)

    #电子银行关系    
    def load_ebank_relation(self):
        #贷款客户号关系
        sql="select org_no||'!'||cust_in_no||'!'||sub_typ,manager_no,typ||hook_type type,percentage from cust_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '电子银行'"
        self.ebankcust_managers=self.get_rela_dict(sql)

    #信用卡关系    
    def load_credit_relation(self):
        sql="select '!'||account_no,manager_no,typ||hook_type type,percentage from account_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '信用卡'"
        self.creditcust_managers=self.get_rela_dict(sql)

    #第三方存管关系    
    def load_stock_relation(self):
        #贷款客户号关系
        sql="select org_no||'!'||cust_in_no,manager_no,typ||hook_type type,percentage from cust_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '第三方存管'"
        self.stockcust_managers=self.get_rela_dict(sql)

    #i代理保险存管关系    
    def load_adb_relation(self):
        #贷款客户号关系
        sql="select org_no||'!'||cust_in_no,manager_no,typ||hook_type type,percentage from cust_hook_his where STATUS in ('待审批','已审批','预提交审批','正常','录入已审批') and typ = '代理保险'"
