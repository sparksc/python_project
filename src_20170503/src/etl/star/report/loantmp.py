...
def man_loan(stardate,etldate):
    try:
        ...
        while stardate<=etldate:
            ...
            FROM F_BALANCE F
            LEFT JOIN (SELECT ACCOUNT_ID,YEAR_PDT,BALANCE FROM F_BALANCE F WHERE DATE_ID=(SELECT L_YEAREND_ID FROM D_DATE  WHERE ID=?) AND ACCT_TYPE=4) F1 ON F1.ACCOUNT_ID=F.ACCOUNT_ID
            ...
            """
            sql1=u"""
            UPDATE YDW.REPORT_MANAGER_LOAN_TEMP SET  PRI_NUM=?, PUB_NUM=?, PRI_LAST_STANDARD_NUM=?,PUB_LAST_STANDARD_NUM=?,PRI_THIS_STANDARD_NUM=?,PUB_THIS_STANDARD_NUM=?,PRI_LAST_AVG=?, PUB_LAST_AVG=?, PRI_THIS_AVG=?, PUB_THIS_AVG=?,PRI_THIS_MON_AVG=?,PUB_THIS_MON_AVG=?, PRI_MONTH_AVG=?,PUB_MONTH_AVG=?,MONTH_AVG=?, MIN_CRD_NUM=?, MIN_NUM=?, MIN_CRD_BAL=?, MIN_BAL=?, BAD_NUM=?, BAD_BAL=?, OUT_NUM=?, OUT_BAL=?,PRI_BAL=?,PUB_BAL=? WHERE   DATE_ID=? AND SALE_CODE=?
            """
            ...
            """更新对公扩面管贷户数.管贷余额 """
            u_sql3="""
            UPDATE REPORT_MANAGER_LOAN_TEMP  SET PUB_ADD_NUM=? ,PUB_ADD_BAL=? WHERE ORG_CODE=? and SALE_CODE=? and DATE_ID=?
            """
            ...
            """更新对私扩面管贷户数.管贷余额 """
            u_sql4="""
            UPDATE REPORT_MANAGER_LOAN_TEMP  SET PRI_ADD_NUM= ?,PRI_ADD_BAL= ? WHERE ORG_CODE=? and SALE_CODE=? and DATE_ID=?
            """
            ...
            """ """
            u_sql5="""
            UPDATE REPORT_MANAGER_LOAN_TEMP  SET TWO_CARD_BY_EBANK= ?,TWO_CARD_ALL=? WHERE  DATE_ID=?  and ORG_CODE=? and SALE_CODE=?
            """
            ...
            """ """
            u_sql6="""
            UPDATE REPORT_MANAGER_LOAN_TEMP  SET TWO_THSI_NUM= ?, TWO_LAST_NUM=?  WHERE  DATE_ID=?  and ORG_CODE=? and SALE_CODE=?
            """
