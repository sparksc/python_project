...
def teller_form_check(request):
    print 'teller_form_check'
    if request.method == 'POST':
        form = request.POST
        teller_serial = form.get('teller_serial')
        teller_branch = form.get('teller_branch')
        try:
            user = User.objects.filter( username = teller_serial, branch = teller_branch ).first()
            if user:
...
def route_manager(request):
    ...
    if branchs is None:
        branchs = ['测试城东银行', '测试城西银行', '测试城北银行' ,'测试城南银行']
